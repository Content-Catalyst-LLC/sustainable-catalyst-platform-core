#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.79.0 validation stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR

BUNDLE="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.79.0-release-bundle.zip}"
select_python(){ local c; for c in python3.12 python3.11 python3.10 python3; do if command -v "$c" >/dev/null 2>&1 && "$c" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3,10) else 1)' >/dev/null 2>&1; then command -v "$c"; return 0; fi; done; echo "ERROR: Platform Core v2.79.0 requires Python 3.10+; Python 3.12 is preferred." >&2; return 1; }
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/sc-platform-core-v2790-bundle.XXXXXX")"
...
REPO_ZIP="sustainable-catalyst-platform-core-v2.79.0-repository.zip"
PLUGIN_ZIP="sustainable-catalyst-platform-core-v2.79.0-wordpress-plugin.zip"
BACKEND_ZIP="sustainable-catalyst-platform-core-backend-v2.79.0.zip"
...
bash -n PUSH_PLATFORM_CORE_V2790_FINAL.sh
bash -n deploy_and_validate_platform_core_v2_79_0_macos.sh
bash -n DEPLOY_PLATFORM_CORE_V2790_CONTABO.sh

if [ "${SC_CORE_BUNDLE_ONLY:-0}" = "1" ]; then
  echo "PASS - v2.79.0 bundle-only verification complete without backend dependencies"
  exit 0
fi
...
TEST_FILES="$(sed 's#^#backend/#' backend/release_v2790_testfiles.txt | tr '\n' ' ')"
backend/.venv/bin/python -m pytest -q $TEST_FILES

SMOKE_DB="$WORK/platform_core_v2790_smoke.db"
rm -f "$SMOKE_DB"
...
backend/.venv/bin/python backend/scripts/migrate.py >/tmp/sc-core-v2790-migrate.json
PYTHONPATH=backend backend/.venv/bin/python backend/scripts/validate_research_arguments.py
backend/.venv/bin/python - <<'PY2'
import json
m = json.load(open('/tmp/sc-core-v2790-migrate.json'))
assert '0083' in m['applied'] and m['pending'] == [], m
print('PASS - migration 0083 applied with pending=[]')
PY2

echo "PASS - v2.79.0 release-critical validation complete"
if [ "${SC_CORE_VALIDATE_ONLY:-0}" = "1" ]; then exit 0; fi
cd "$WORK/bundle"
chmod +x PUSH_PLATFORM_CORE_V2790_FINAL.sh
PYTHON_BIN="$PYTHON_BIN" ./PUSH_PLATFORM_CORE_V2790_FINAL.sh "$WORK/bundle/$REPO_ZIP"

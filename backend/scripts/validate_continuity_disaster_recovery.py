...
def version_tuple(value): return tuple(int(x) for x in value.split('.')[:3])
def main():
    ...
    try:
        ...
        with d.session_factory() as db:
            ...
            assert version_tuple(s.version)>=(2,20,0) and '0023' in migration_status(d)['applied'] and not migration_status(d)['pending']
        ...
        print(f'PASS - Core {s.version} continuity backup verification and disaster recovery validation')
    finally:
        try: os.remove(p)
        except OSError: pass

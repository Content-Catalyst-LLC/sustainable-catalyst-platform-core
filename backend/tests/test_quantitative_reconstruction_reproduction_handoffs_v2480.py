...
def app_client(tmp_path):
    app=create_app(Settings(database_url=f"sqlite:///{tmp_path/'v2480.db'}",version='2.49.0'))
...
def test_v2480_readiness_boundaries_and_migration(tmp_path):
    ...
    h=c.get('/health').json(); assert h['version']=='2.49.0' and h['forensic_quantitative_reconstruction_reproduction_handoffs'] is True
    r=c.get('/v1/open-forensics/readiness').json(); assert r['release']=='2.49.0' and r['migration_0052_applied'] is True

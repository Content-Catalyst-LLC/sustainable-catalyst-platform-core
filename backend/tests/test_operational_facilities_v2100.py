...
def test_release_and_migration(client):
    assert client.get('/health').json()['version']=='2.10.0'
    ready=client.get('/v1/facilities/readiness').json(); assert ready['release']=='2.10.0'; assert ready['migration_0013_applied'] is True; assert ready['history_preserving'] is True; assert ready['automatic_conflict_flattening'] is False

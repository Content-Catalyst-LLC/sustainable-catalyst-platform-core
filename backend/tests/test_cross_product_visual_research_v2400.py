...
def app_client(tmp_path):
    app = create_app(Settings(database_url=f"sqlite:///{tmp_path/'v2400.db'}", version="2.40.0"))
...
def test_readiness_health_and_migration_0044(tmp_path):
    ...
    assert health["version"] == "2.40.0" and health["cross_product_visual_research_objects"] is True
    ready = client.get("/v1/cross-product-visual-research/readiness").json()
    assert ready["release"] == "2.40.0" and ready["migration_0044_applied"] is True

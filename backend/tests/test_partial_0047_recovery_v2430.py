...
def test_partial_0047_table_state_recovers_by_recording_metadata(tmp_path):
    ...
    assert run_migrations(db)==['0047']; status=migration_status(db); assert status['pending']==[] and status['applied'][-1]=='0047'
...
def test_pristine_pre_0047_schema_upgrades_additively(tmp_path):
    ...
    _ledger_to(db,'0047'); assert run_migrations(db)==['0047']; status=migration_status(db); assert status['pending']==[] and status['applied'][-1]=='0047'; assert TABLES <= set(inspect(db.engine).get_table_names())

...
def test_readiness_and_migration_0022():
    with TemporaryDirectory() as td:
        app,c=app_client(td); b=c.get('/v1/operations/readiness').json(); assert b['release']=='2.28.0' and b['migration_0022_applied'] and b['automatic_rollback_enabled'] is False
...
def test_high_risk_change_requires_approval_before_start():
    with TemporaryDirectory() as td:
        app,c=app_client(td)
        with app.state.database.session_factory() as db:
            row=operations.create_change(db,app.state.settings,change_key='high-1',risk='high',release='2.28.0'); assert row.approval_required
...
def test_no_breach_does_not_auto_recommend_rollback():
    with TemporaryDirectory() as td:
        app,c=app_client(td)
        with app.state.database.session_factory() as db:
            inc=operations.create_incident(db,title='issue',severity='sev2'); dep=observability.create_deployment_marker(db,release='2.28.0',environment='production',state='deployed'); rb=operations.assess_rollback(db,incident=inc,deployment_marker_id=dep.id,slo_evaluations=[{'state':'met'}]); assert rb.recommendation=='review'

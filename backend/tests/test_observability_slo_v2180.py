...
def test_health_and_readiness_expose_observability():
    with TemporaryDirectory() as td:
        app,c=app_client(td)
        assert c.get('/health').json()['version']=='2.35.0'
        ...
        b=r.json(); assert b['release']=='2.35.0' and b['migration_0021_applied'] is True and b['external_monitoring_provider_required'] is False
...
def test_deployment_markers_preserve_release_history():
    with TemporaryDirectory() as td:
        app,c=app_client(td, observability_request_metrics_enabled=False)
        with app.state.database.session_factory() as db:
            observability.create_deployment_marker(db,release='2.35.0',environment='test',state='started',commit_sha='abc')
            observability.create_deployment_marker(db,release='2.35.0',environment='test',state='deployed',commit_sha='def')
...
def test_invalid_deployment_state_is_rejected():
    with TemporaryDirectory() as td:
        app,c=app_client(td, observability_request_metrics_enabled=False)
        with app.state.database.session_factory() as db:
            try: observability.create_deployment_marker(db,release='2.35.0',environment='test',state='magic')
            except ValueError: pass

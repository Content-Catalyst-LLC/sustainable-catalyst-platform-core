...
def test_v234_health_migration_and_readiness(client):
    ...
    assert health['version']==client.app.state.settings.version and health['interactive_model_canvas'] is True
    ready=client.get('/v1/model-canvases/readiness').json()
    assert ready['release']==client.app.state.settings.version and ready['migration_0037_applied'] is True

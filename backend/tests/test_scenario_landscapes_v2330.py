...
def test_v233_health_migration_and_readiness(client):
    assert any(v=='0036' for v,_ in MIGRATIONS);health=client.get('/health').json();assert health['version']=='2.52.0' and health['scenario_landscapes'] is True
    ready=client.get('/v1/scenario-landscapes/readiness').json();assert ready['release']=='2.52.0' and ready['migration_0036_applied'] is True and ready['visual_kind']=='scenario-landscape';assert ready['research_scenarios_reused'] is True;assert ready['scenario_execution_by_core'] is False and ready['ranking_by_core'] is False and ready['automatic_truth_promotion'] is False

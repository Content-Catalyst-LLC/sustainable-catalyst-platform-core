...
def test_readiness_and_migration_0024():
    ...
    try:
        app=create_app(Settings(database_url='sqlite:///'+p,observability_request_metrics_enabled=False)); c=TestClient(app); b=c.get('/v1/resilience/readiness').json(); assert b['release']=='2.28.0' and b['migration_0024_applied'] and b['automatic_failover_enabled'] is False and b['infrastructure_actuation_by_core'] is False
    finally: os.unlink(p)

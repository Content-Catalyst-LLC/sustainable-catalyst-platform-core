...
def test_migration_0041_metadata_is_repair_safe():
    description = dict(MIGRATIONS)["0041"]
    assert len(description) <= 300
    assert "causal systems explorer" in description.lower()
    assert "lab/workbench" in description.lower()

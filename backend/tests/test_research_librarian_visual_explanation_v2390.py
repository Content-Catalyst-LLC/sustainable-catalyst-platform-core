...
def app_client(tmp_path):
    app = create_app(Settings(database_url=f"sqlite:///{tmp_path/'v2390.db'}", version="2.53.0"))
...
def test_readiness_health_and_migration_0043(tmp_path):
    ...
    health = client.get("/health").json(); assert health["version"] == "2.53.0"; assert health["research_librarian_visual_explanation"] is True
    ready = client.get("/v1/research-visual-explanations/readiness").json()
    assert ready["release"] == "2.53.0"; assert ready["migration_0043_applied"] is True

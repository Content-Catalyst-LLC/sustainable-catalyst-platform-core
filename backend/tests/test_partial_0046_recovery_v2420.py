...
def test_partial_0046_table_state_recovers_by_recording_metadata(tmp_path):
    ...
    assert run_migrations(db)==['0046','0047','0048','0049','0050','0051','0052','0053','0054','0055','0056','0057','0058']
...
def test_pristine_pre_0046_schema_upgrades_additively(tmp_path):
    ...
    with db.session_factory() as session:
        for version,description in MIGRATIONS:
            ...
            session.add(SchemaMigration(version=version,description=description))
        session.commit()
    assert run_migrations(db)==['0046','0047','0048','0049','0050','0051','0052','0053','0054','0055','0056','0057','0058']; status=migration_status(db); assert status['pending']==[] and status['applied'][-1]=='0058'; assert TABLES <= set(inspect(db.engine).get_table_names())

...
def app_client(tmp_path):
    return TestClient(create_app(Settings(database_url='sqlite:///'+str(tmp_path/'v237.db'),version='2.41.0')))
...
def test_readiness_and_migration(tmp_path):
    c=app_client(tmp_path); h=c.get('/health').json(); assert h['version']=='2.41.0' and h['causal_systems_explorer'] is True

...
def test_migration_head_and_readiness():
 d,p=db()
 try:
  s=Settings(database_url='sqlite:///'+p); m=certification.migration_assurance(d); assert m['schema_head']==MIGRATIONS[-1][0] and m['zero_pending'] and m['head_matches']

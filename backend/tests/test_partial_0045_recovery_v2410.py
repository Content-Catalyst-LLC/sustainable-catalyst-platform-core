...
def test_partial_0045_table_state_recovers_by_recording_metadata(tmp_path):
    ...
    assert run_migrations(db)==['0045','0046','0047','0048','0049']
    with db.session_factory() as session:
        row=session.get(SchemaMigration,'0045'); assert row and len(row.description)<=300
    status=migration_status(db); assert status['pending']==[] and status['applied'][-1]=='0049'
...
def test_pristine_pre_0045_schema_upgrades_additively(tmp_path):
    ...
    with db.session_factory() as session:
        for version,description in MIGRATIONS:
            ...
            session.add(SchemaMigration(version=version,description=description))
        session.commit()
    assert run_migrations(db)==['0045','0046','0047','0048','0049']; status=migration_status(db); assert status['pending']==[] and status['applied'][-1]=='0049'; assert TABLES <= set(inspect(db.engine).get_table_names())

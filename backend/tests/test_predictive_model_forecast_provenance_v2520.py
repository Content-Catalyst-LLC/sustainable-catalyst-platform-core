...
def app_client(tmp_path):
    app=create_app(Settings(database_url=f"sqlite:///{tmp_path/'v2520.db'}",version='2.54.0')); return app,TestClient(app)
def seed(app):
...
def test_v2520_readiness_and_boundaries(tmp_path):
    app,c=app_client(tmp_path); seed(app); r=c.get('/v1/predictive-intelligence/readiness'); assert r.status_code==200,r.text; d=r.json(); assert d['release']=='2.54.0' and d['migration_0056_applied'] is True

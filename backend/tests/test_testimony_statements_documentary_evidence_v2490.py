...
def app_client(tmp_path):
    app=create_app(Settings(database_url=f"sqlite:///{tmp_path/'v2490.db'}",version='2.53.0'))
...
def test_v2490_readiness_boundaries_and_migration(tmp_path):
    ...
    h=c.get('/health').json(); assert h['version']=='2.53.0' and h['forensic_testimony_statements_documentary_evidence'] is True
    r=c.get('/v1/open-forensics/readiness').json(); assert r['release']=='2.53.0' and r['migration_0053_applied'] is True

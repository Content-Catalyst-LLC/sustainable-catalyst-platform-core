...
def app_client(tmp_path):
    app=create_app(Settings(database_url=f"sqlite:///{tmp_path/'v2470.db'}",version='2.47.0'))
...
def test_v2470_readiness_boundaries_and_migration(tmp_path):
    ...
    h=c.get('/health').json(); assert h['version']=='2.47.0' and h['forensic_media_artifact_derivative_provenance'] is True
    r=c.get('/v1/open-forensics/readiness').json(); assert r['release']=='2.47.0' and r['migration_0051_applied'] is True

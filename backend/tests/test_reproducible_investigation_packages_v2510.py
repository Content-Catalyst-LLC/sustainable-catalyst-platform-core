...
def app_client(tmp_path):
    app=create_app(Settings(database_url=f"sqlite:///{tmp_path/'v2510.db'}",version='2.54.0')); return app,TestClient(app)
...
def test_v2510_readiness_and_boundaries(tmp_path):
    app,c=app_client(tmp_path); seed(app); r=c.get('/v1/open-forensics/readiness').json(); assert r['release']=='2.54.0' and r['migration_0055_applied'] is True

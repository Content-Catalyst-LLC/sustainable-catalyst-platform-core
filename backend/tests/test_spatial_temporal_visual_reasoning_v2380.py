...
def app_client(tmp_path):
    app=create_app(Settings(database_url=f"sqlite:///{tmp_path/'v2380.db'}",version='2.39.0'))
...
def test_readiness_and_migration_0042(tmp_path):
    ...
    d=c.get('/v1/spatial-temporal/readiness').json(); assert d['release']=='2.39.0'; assert d['migration_0042_applied'] is True; assert d['spatial_analysis_by_core'] is False; assert d['automatic_truth_promotion'] is False

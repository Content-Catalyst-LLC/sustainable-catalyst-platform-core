...
from .models import (
    ...
    EnsembleRecord, EnsembleMemberRecord, EnsembleStatisticRecord,
...
MIGRATIONS = [
    ...
    ("0039", "First-class uncertainty definitions, governed sensitivity-study factors and externally supplied metrics, ensemble membership and externally supplied statistics, validation, provenance, and renderer-neutral reasoning views without numerical execution by Core."),
    ("0040", "Reproducible uncertainty compute runtime integration with deterministic Monte Carlo and Latin Hypercube sampling, Sobol/Morris design and analysis helpers, ensemble weight normalization and descriptive statistics, empirical exceedance probabilities, and governed Lab/Workbench execution handoff manifests."),
]
...
DEFAULT_RENDERER_COMPATIBILITY_RULES = [
    ...
    ("contract.vega-lite", "sensitivity-map", "chart", 340), ("contract.plotly", "sensitivity-map", "chart", 330),
    ("contract.vega-lite", "ensemble-view", "chart", 330), ("contract.plotly", "ensemble-view", "chart", 340),
...
def migration_status(database: Database) -> dict:
    ...
    with database.session_factory() as session:
        ...
        sensitivity_studies = len(session.scalars(select(SensitivityStudyRecord.visual_entity_id)).all())
        sensitivity_factors = len(session.scalars(select(SensitivityFactorRecord.id)).all())
        sensitivity_results = len(session.scalars(select(SensitivityResultRecord.id)).all())
        ensembles = len(session.scalars(select(EnsembleRecord.visual_entity_id)).all())

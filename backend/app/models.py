...
class ComputationExecutionRecord(Base):
    __tablename__ = "computation_executions_v287"
    __table_args__ = (UniqueConstraint("execution_key", name="uq_computation_execution_v287_key"), Index("ix_computation_execution_v287_runtime_status", "runtime_kind", "status"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    ...
    runtime_kind: Mapped[str] = mapped_column(String(80), nullable=False)
    status: Mapped[str] = mapped_column(String(60), nullable=False, default="recorded")
    visibility: Mapped[str] = mapped_column(String(30), nullable=False, default="private")
    project_ref: Mapped[str | None] = mapped_column(String(255), nullable=True)
    protocol_id: Mapped[str | None] = mapped_column(ForeignKey("scientific_research_protocols_v286.id", ondelete="SET NULL"), nullable=True)
    method_plan_ref: Mapped[str | None] = mapped_column(String(255), nullable=True)
    external_run_ref: Mapped[str | None] = mapped_column(String(500), nullable=True)
    command_or_entrypoint: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class ComputationExecutionInputRecord(Base):
    __tablename__ = "computation_execution_inputs_v287"
    __table_args__ = (UniqueConstraint("execution_id", "input_key", name="uq_computation_input_v287_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    execution_id: Mapped[str] = mapped_column(ForeignKey("computation_executions_v287.id", ondelete="CASCADE"), nullable=False)
    input_key: Mapped[str] = mapped_column(String(180), nullable=False)
    input_type: Mapped[str] = mapped_column(String(100), nullable=False)
...
class ComputationExecutionParameterRecord(Base):
    __tablename__ = "computation_execution_parameters_v287"
    __table_args__ = (UniqueConstraint("execution_id", "parameter_key", name="uq_computation_parameter_v287_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    execution_id: Mapped[str] = mapped_column(ForeignKey("computation_executions_v287.id", ondelete="CASCADE"), nullable=False)
    parameter_key: Mapped[str] = mapped_column(String(180), nullable=False)
    value_json: Mapped[dict] = mapped_column(JSON, default=dict)
    unit: Mapped[str | None] = mapped_column(String(120), nullable=True)
    source_ref: Mapped[str | None] = mapped_column(String(500), nullable=True)
    sensitivity_role: Mapped[str | None] = mapped_column(String(100), nullable=True)
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)

class ComputationExecutionAssumptionRecord(Base):
    __tablename__ = "computation_execution_assumptions_v287"
    __table_args__ = (UniqueConstraint("execution_id", "assumption_key", name="uq_computation_assumption_v287_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    execution_id: Mapped[str] = mapped_column(ForeignKey("computation_executions_v287.id", ondelete="CASCADE"), nullable=False)
    assumption_key: Mapped[str] = mapped_column(String(180), nullable=False)
    statement_text: Mapped[str] = mapped_column(Text, nullable=False)
    protocol_assumption_ref: Mapped[str | None] = mapped_column(String(255), nullable=True)
    evidence_refs_json: Mapped[list] = mapped_column(JSON, default=list)
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)

class ComputationExecutionEnvironmentRecord(Base):
    __tablename__ = "computation_execution_environments_v287"
    __table_args__ = (UniqueConstraint("execution_id", "environment_key", name="uq_computation_environment_v287_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    execution_id: Mapped[str] = mapped_column(ForeignKey("computation_executions_v287.id", ondelete="CASCADE"), nullable=False)
    environment_key: Mapped[str] = mapped_column(String(180), nullable=False)
    environment_type: Mapped[str] = mapped_column(String(100), nullable=False)
    runtime_name: Mapped[str | None] = mapped_column(String(180), nullable=True)
    runtime_version: Mapped[str | None] = mapped_column(String(180), nullable=True)
    os_arch: Mapped[str | None] = mapped_column(String(255), nullable=True)
    ...
    dependency_manifest_ref: Mapped[str | None] = mapped_column(String(500), nullable=True)
...
class ComputationExecutionStepRecord(Base):
    __tablename__ = "computation_execution_steps_v287"
    __table_args__ = (UniqueConstraint("execution_id", "step_key", name="uq_computation_step_v287_key"), Index("ix_computation_step_v287_sequence", "execution_id", "sequence"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    execution_id: Mapped[str] = mapped_column(ForeignKey("computation_executions_v287.id", ondelete="CASCADE"), nullable=False)
    step_key: Mapped[str] = mapped_column(String(180), nullable=False)
    sequence: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    step_type: Mapped[str] = mapped_column(String(100), nullable=False)
    tool_ref: Mapped[str | None] = mapped_column(String(500), nullable=True)
    operation_text: Mapped[str | None] = mapped_column(Text, nullable=True)
    code_ref: Mapped[str | None] = mapped_column(String(500), nullable=True)
    input_refs_json: Mapped[list] = mapped_column(JSON, default=list)
    output_refs_json: Mapped[list] = mapped_column(JSON, default=list)
    parameters_json: Mapped[dict] = mapped_column(JSON, default=dict)
...
class ComputationExecutionOutputRecord(Base):
    __tablename__ = "computation_execution_outputs_v287"
    __table_args__ = (UniqueConstraint("execution_id", "output_key", name="uq_computation_output_v287_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    execution_id: Mapped[str] = mapped_column(ForeignKey("computation_executions_v287.id", ondelete="CASCADE"), nullable=False)
    output_key: Mapped[str] = mapped_column(String(180), nullable=False)
    output_type: Mapped[str] = mapped_column(String(100), nullable=False)
...
class ComputationResearchBindingRecord(Base):
    __tablename__ = "computation_research_bindings_v287"
    __table_args__ = (UniqueConstraint("execution_id", "binding_key", name="uq_computation_research_binding_v287_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    execution_id: Mapped[str] = mapped_column(ForeignKey("computation_executions_v287.id", ondelete="CASCADE"), nullable=False)
    binding_key: Mapped[str] = mapped_column(String(180), nullable=False)
    source_output_ref: Mapped[str | None] = mapped_column(String(500), nullable=True)
    ...
    binding_role: Mapped[str | None] = mapped_column(String(120), nullable=True)
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)

class ComputationExecutionDependencyRecord(Base):
    __tablename__ = "computation_execution_dependencies_v287"
    __table_args__ = (UniqueConstraint("execution_id", "dependency_key", name="uq_computation_dependency_v287_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    execution_id: Mapped[str] = mapped_column(ForeignKey("computation_executions_v287.id", ondelete="CASCADE"), nullable=False)
    dependency_key: Mapped[str] = mapped_column(String(180), nullable=False)
    upstream_execution_ref: Mapped[str] = mapped_column(String(500), nullable=False)
    upstream_output_ref: Mapped[str | None] = mapped_column(String(500), nullable=True)
    relation: Mapped[str] = mapped_column(String(120), nullable=False)
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)

class ComputationExecutionVerificationRecord(Base):
    __tablename__ = "computation_execution_verifications_v287"
    __table_args__ = (UniqueConstraint("execution_id", "verification_key", name="uq_computation_verification_v287_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    execution_id: Mapped[str] = mapped_column(ForeignKey("computation_executions_v287.id", ondelete="CASCADE"), nullable=False)
    verification_key: Mapped[str] = mapped_column(String(180), nullable=False)
...
class ComputationExecutionRevisionRecord(Base):
    __tablename__ = "computation_execution_revisions_v287"
    __table_args__ = (UniqueConstraint("execution_id", "revision", name="uq_computation_revision_v287_revision"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    execution_id: Mapped[str] = mapped_column(ForeignKey("computation_executions_v287.id", ondelete="CASCADE"), nullable=False)
    revision: Mapped[int] = mapped_column(Integer, nullable=False)
    state_hash: Mapped[str] = mapped_column(String(64), nullable=False)
...
class ComputationExecutionSnapshotRecord(Base):
    __tablename__ = "computation_execution_snapshots_v287"
    __table_args__ = (UniqueConstraint("execution_id", "revision", name="uq_computation_snapshot_v287_revision"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    execution_id: Mapped[str] = mapped_column(ForeignKey("computation_executions_v287.id", ondelete="CASCADE"), nullable=False)
    revision: Mapped[int] = mapped_column(Integer, nullable=False)
    content_hash: Mapped[str] = mapped_column(String(64), nullable=False)
...
class ResearchInferenceRecord(Base):
    __tablename__ = "research_inferences_v288"
    __table_args__ = (
        UniqueConstraint("inference_key", name="uq_research_inference_v288_key"),
        Index("ix_research_inference_v288_project_type", "project_ref", "inference_type"),
        Index("ix_research_inference_v288_status", "status"),
    )
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    inference_key: Mapped[str] = mapped_column(String(180), nullable=False)
    title: Mapped[str] = mapped_column(String(500), nullable=False)
    statement_text: Mapped[str] = mapped_column(Text, nullable=False)
    inference_type: Mapped[str] = mapped_column(String(100), nullable=False)
    status: Mapped[str] = mapped_column(String(60), nullable=False, default="proposed")
    visibility: Mapped[str] = mapped_column(String(30), nullable=False, default="private")
    ...
    method_ref: Mapped[str | None] = mapped_column(String(500), nullable=True)
    source_object_type: Mapped[str | None] = mapped_column(String(100), nullable=True)
    source_object_ref: Mapped[str | None] = mapped_column(String(500), nullable=True)
    scope_json: Mapped[dict] = mapped_column(JSON, default=dict)
    confidence_declaration_json: Mapped[dict] = mapped_column(JSON, default=dict)
    qualifications_json: Mapped[list] = mapped_column(JSON, default=list)
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    metadata_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
...
class ResearchInferenceClassificationRecord(Base):
    __tablename__ = "research_inference_classifications_v288"
    __table_args__ = (UniqueConstraint("inference_id", "classification_key", name="uq_research_inference_classification_v288_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    inference_id: Mapped[str] = mapped_column(ForeignKey("research_inferences_v288.id", ondelete="CASCADE"), nullable=False)
    classification_key: Mapped[str] = mapped_column(String(180), nullable=False)
    object_type: Mapped[str] = mapped_column(String(100), nullable=False)
    object_ref: Mapped[str] = mapped_column(String(500), nullable=False)
    inference_type: Mapped[str] = mapped_column(String(100), nullable=False)
    classification_basis: Mapped[str | None] = mapped_column(Text, nullable=True)
    declared_by: Mapped[str | None] = mapped_column(String(255), nullable=True)
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)

class ResearchInferenceBasisRecord(Base):
    __tablename__ = "research_inference_basis_bindings_v288"
    __table_args__ = (UniqueConstraint("inference_id", "basis_key", name="uq_research_inference_basis_v288_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    inference_id: Mapped[str] = mapped_column(ForeignKey("research_inferences_v288.id", ondelete="CASCADE"), nullable=False)
    basis_key: Mapped[str] = mapped_column(String(180), nullable=False)
    basis_type: Mapped[str] = mapped_column(String(100), nullable=False)
    basis_ref: Mapped[str] = mapped_column(String(500), nullable=False)
    relation: Mapped[str] = mapped_column(String(120), nullable=False)
...
class ResearchInferenceAssumptionRecord(Base):
    __tablename__ = "research_inference_assumptions_v288"
    __table_args__ = (UniqueConstraint("inference_id", "assumption_key", name="uq_research_inference_assumption_v288_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    inference_id: Mapped[str] = mapped_column(ForeignKey("research_inferences_v288.id", ondelete="CASCADE"), nullable=False)
    assumption_key: Mapped[str] = mapped_column(String(180), nullable=False)
    statement_text: Mapped[str] = mapped_column(Text, nullable=False)
    assumption_type: Mapped[str] = mapped_column(String(100), nullable=False, default="declared")
    source_ref: Mapped[str | None] = mapped_column(String(500), nullable=True)
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)

class ResearchInferenceUncertaintyRecord(Base):
    __tablename__ = "research_inference_uncertainty_v288"
    __table_args__ = (UniqueConstraint("inference_id", "uncertainty_key", name="uq_research_inference_uncertainty_v288_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    inference_id: Mapped[str] = mapped_column(ForeignKey("research_inferences_v288.id", ondelete="CASCADE"), nullable=False)
    uncertainty_key: Mapped[str] = mapped_column(String(180), nullable=False)
    uncertainty_type: Mapped[str] = mapped_column(String(100), nullable=False)
    description: Mapped[str] = mapped_column(Text, nullable=False)
    quantitative_json: Mapped[dict] = mapped_column(JSON, default=dict)
    source_ref: Mapped[str | None] = mapped_column(String(500), nullable=True)
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)

class ResearchInferenceRelationRecord(Base):
    __tablename__ = "research_inference_relations_v288"
    __table_args__ = (UniqueConstraint("inference_id", "relation_key", name="uq_research_inference_relation_v288_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    inference_id: Mapped[str] = mapped_column(ForeignKey("research_inferences_v288.id", ondelete="CASCADE"), nullable=False)
    relation_key: Mapped[str] = mapped_column(String(180), nullable=False)
    target_inference_ref: Mapped[str] = mapped_column(String(500), nullable=False)
    relation: Mapped[str] = mapped_column(String(120), nullable=False)
    rationale: Mapped[str | None] = mapped_column(Text, nullable=True)
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
...
class ResearchInferenceChallengeRecord(Base):
    __tablename__ = "research_inference_challenges_v288"
    __table_args__ = (UniqueConstraint("inference_id", "challenge_key", name="uq_research_inference_challenge_v288_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    inference_id: Mapped[str] = mapped_column(ForeignKey("research_inferences_v288.id", ondelete="CASCADE"), nullable=False)
    challenge_key: Mapped[str] = mapped_column(String(180), nullable=False)
...
class ResearchInferenceRevisionRecord(Base):
    __tablename__ = "research_inference_revisions_v288"
    __table_args__ = (UniqueConstraint("inference_id", "revision", name="uq_research_inference_revision_v288_revision"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    inference_id: Mapped[str] = mapped_column(ForeignKey("research_inferences_v288.id", ondelete="CASCADE"), nullable=False)
    revision: Mapped[int] = mapped_column(Integer, nullable=False)
    state_hash: Mapped[str] = mapped_column(String(64), nullable=False)
...
class ResearchInferenceSnapshotRecord(Base):
    __tablename__ = "research_inference_snapshots_v288"
    __table_args__ = (UniqueConstraint("inference_id", "revision", name="uq_research_inference_snapshot_v288_revision"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    inference_id: Mapped[str] = mapped_column(ForeignKey("research_inferences_v288.id", ondelete="CASCADE"), nullable=False)
    revision: Mapped[int] = mapped_column(Integer, nullable=False)
    content_hash: Mapped[str] = mapped_column(String(64), nullable=False)
...
class ResearchContributorRecord(Base):
    __tablename__ = "research_contributors_v293"
    __table_args__ = (UniqueConstraint("contributor_key", name="uq_research_contributor_v293_key"), Index("ix_research_contributor_v293_type_status", "actor_type", "status"))
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    contributor_key: Mapped[str] = mapped_column(String(180), nullable=False)
    display_name: Mapped[str] = mapped_column(String(500), nullable=False)
    actor_type: Mapped[str] = mapped_column(String(80), nullable=False)
    identity_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    affiliation_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    status: Mapped[str] = mapped_column(String(60), nullable=False, default="active")
    visibility: Mapped[str] = mapped_column(String(30), nullable=False, default="private")
    metadata_json: Mapped[dict] = mapped_column(JSON, default=dict)
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
...
class ResearchRoleDefinitionRecord(Base):
    __tablename__ = "research_role_definitions_v293"
    __table_args__ = (UniqueConstraint("role_key", name="uq_research_role_definition_v293_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    role_key: Mapped[str] = mapped_column(String(180), nullable=False)
    label: Mapped[str] = mapped_column(String(255), nullable=False)
    role_category: Mapped[str] = mapped_column(String(80), nullable=False)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    taxonomy_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    capabilities_json: Mapped[dict] = mapped_column(JSON, default=dict)
...
class ResearchRoleAssignmentRecord(Base):
    __tablename__ = "research_role_assignments_v293"
    __table_args__ = (UniqueConstraint("assignment_key", name="uq_research_role_assignment_v293_key"), Index("ix_research_role_assignment_v293_scope", "scope_type", "scope_ref"))
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    assignment_key: Mapped[str] = mapped_column(String(180), nullable=False)
    contributor_id: Mapped[str] = mapped_column(ForeignKey("research_contributors_v293.id", ondelete="CASCADE"), nullable=False)
    role_definition_id: Mapped[str] = mapped_column(ForeignKey("research_role_definitions_v293.id", ondelete="CASCADE"), nullable=False)
    scope_type: Mapped[str] = mapped_column(String(100), nullable=False)
    scope_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    status: Mapped[str] = mapped_column(String(60), nullable=False, default="declared")
    delegated_by_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
...
class ResearchContributionRecord(Base):
    __tablename__ = "research_contributions_v293"
    __table_args__ = (UniqueConstraint("contribution_key", name="uq_research_contribution_v293_key"), Index("ix_research_contribution_v293_project", "project_ref", "contribution_type"))
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    contribution_key: Mapped[str] = mapped_column(String(180), nullable=False)
    contributor_id: Mapped[str] = mapped_column(ForeignKey("research_contributors_v293.id", ondelete="CASCADE"), nullable=False)
    role_assignment_id: Mapped[str | None] = mapped_column(ForeignKey("research_role_assignments_v293.id", ondelete="SET NULL"), nullable=True)
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    contribution_type: Mapped[str] = mapped_column(String(120), nullable=False)
...
class ResearchAgentProfileRecord(Base):
    __tablename__ = "research_agent_profiles_v293"
    __table_args__ = (UniqueConstraint("agent_key", name="uq_research_agent_profile_v293_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    agent_key: Mapped[str] = mapped_column(String(180), nullable=False)
    contributor_id: Mapped[str] = mapped_column(ForeignKey("research_contributors_v293.id", ondelete="CASCADE"), nullable=False)
    specialist_role: Mapped[str] = mapped_column(String(160), nullable=False)
    provider_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    model_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    configuration_hash: Mapped[str | None] = mapped_column(String(64), nullable=True)
    autonomy_level: Mapped[str] = mapped_column(String(80), nullable=False, default="human_supervised")
    human_supervisor_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    allowed_actions_json: Mapped[list] = mapped_column(JSON, default=list)
    prohibited_actions_json: Mapped[list] = mapped_column(JSON, default=list)
    status: Mapped[str] = mapped_column(String(60), nullable=False, default="declared")
    visibility: Mapped[str] = mapped_column(String(30), nullable=False, default="internal")
    metadata_json: Mapped[dict] = mapped_column(JSON, default=dict)
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
...
class ResearchAgentActionRecord(Base):
    __tablename__ = "research_agent_actions_v293"
    __table_args__ = (UniqueConstraint("action_key", name="uq_research_agent_action_v293_key"), Index("ix_research_agent_action_v293_project", "project_ref", "action_type"))
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    action_key: Mapped[str] = mapped_column(String(180), nullable=False)
    agent_profile_id: Mapped[str] = mapped_column(ForeignKey("research_agent_profiles_v293.id", ondelete="CASCADE"), nullable=False)
    contribution_id: Mapped[str | None] = mapped_column(ForeignKey("research_contributions_v293.id", ondelete="SET NULL"), nullable=True)
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    action_type: Mapped[str] = mapped_column(String(160), nullable=False)
    inputs_json: Mapped[list] = mapped_column(JSON, default=list)
    outputs_json: Mapped[list] = mapped_column(JSON, default=list)
    ...
    runtime_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    model_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    decision_basis_json: Mapped[dict] = mapped_column(JSON, default=dict)
    human_review_status: Mapped[str] = mapped_column(String(80), nullable=False, default="not_reviewed")
    visibility: Mapped[str] = mapped_column(String(30), nullable=False, default="internal")
    metadata_json: Mapped[dict] = mapped_column(JSON, default=dict)
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
...
class ResearchAuthorshipAssertionRecord(Base):
    __tablename__ = "research_authorship_assertions_v293"
    __table_args__ = (UniqueConstraint("assertion_key", name="uq_research_authorship_assertion_v293_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    assertion_key: Mapped[str] = mapped_column(String(180), nullable=False)
    contributor_id: Mapped[str] = mapped_column(ForeignKey("research_contributors_v293.id", ondelete="CASCADE"), nullable=False)
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    publication_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    authorship_type: Mapped[str] = mapped_column(String(120), nullable=False)
    position_label: Mapped[str | None] = mapped_column(String(120), nullable=True)
    contribution_statement: Mapped[str] = mapped_column(Text, nullable=False)
...
class ResearchResponsibilityStatementRecord(Base):
    __tablename__ = "research_responsibility_statements_v293"
    __table_args__ = (UniqueConstraint("statement_key", name="uq_research_responsibility_statement_v293_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    statement_key: Mapped[str] = mapped_column(String(180), nullable=False)
    contributor_id: Mapped[str] = mapped_column(ForeignKey("research_contributors_v293.id", ondelete="CASCADE"), nullable=False)
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    scope_type: Mapped[str] = mapped_column(String(120), nullable=False)
    scope_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    responsibility_type: Mapped[str] = mapped_column(String(160), nullable=False)
    statement: Mapped[str] = mapped_column(Text, nullable=False)
    acceptance_status: Mapped[str] = mapped_column(String(80), nullable=False, default="declared")
    visibility: Mapped[str] = mapped_column(String(30), nullable=False, default="internal")
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)

class ResearchContributionReviewRecord(Base):
    __tablename__ = "research_contribution_reviews_v293"
    __table_args__ = (UniqueConstraint("review_key", name="uq_research_contribution_review_v293_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    review_key: Mapped[str] = mapped_column(String(180), nullable=False)
    contribution_id: Mapped[str] = mapped_column(ForeignKey("research_contributions_v293.id", ondelete="CASCADE"), nullable=False)
    reviewer_contributor_id: Mapped[str] = mapped_column(ForeignKey("research_contributors_v293.id", ondelete="CASCADE"), nullable=False)
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
...
class ResearchContributorRevisionRecord(Base):
    __tablename__ = "research_contributor_revisions_v293"
    __table_args__ = (UniqueConstraint("project_ref", "revision", name="uq_research_contributor_revision_v293"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    revision: Mapped[int] = mapped_column(Integer, nullable=False)
    contributor_id: Mapped[str | None] = mapped_column(ForeignKey("research_contributors_v293.id", ondelete="SET NULL"), nullable=True)
...
class ResearchContributorSnapshotRecord(Base):
    __tablename__ = "research_contributor_snapshots_v293"
    __table_args__ = (UniqueConstraint("project_ref", "revision", name="uq_research_contributor_snapshot_v293_revision"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    revision: Mapped[int] = mapped_column(Integer, nullable=False)
    content_hash: Mapped[str] = mapped_column(String(64), nullable=False)
...
class ResearchValidationChallengeRecord(Base):
    __tablename__ = "research_validation_challenges_v294"
    __table_args__ = (UniqueConstraint("challenge_key", name="uq_research_validation_challenge_v294_key"), Index("ix_research_validation_challenge_v294_project", "project_ref", "challenge_type"))
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    challenge_key: Mapped[str] = mapped_column(String(180), nullable=False)
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    challenge_type: Mapped[str] = mapped_column(String(120), nullable=False)
    title: Mapped[str] = mapped_column(String(500), nullable=False)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    status: Mapped[str] = mapped_column(String(80), nullable=False, default="open")
    source_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    raised_by_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    visibility: Mapped[str] = mapped_column(String(30), nullable=False, default="internal")
    metadata_json: Mapped[dict] = mapped_column(JSON, default=dict)
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
...
class ResearchValidationTargetRecord(Base):
    __tablename__ = "research_validation_targets_v294"
    __table_args__ = (UniqueConstraint("target_key", name="uq_research_validation_target_v294_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    target_key: Mapped[str] = mapped_column(String(180), nullable=False)
    challenge_id: Mapped[str] = mapped_column(ForeignKey("research_validation_challenges_v294.id", ondelete="CASCADE"), nullable=False)
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    target_type: Mapped[str] = mapped_column(String(120), nullable=False)
    target_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    target_version_ref: Mapped[str | None] = mapped_column(String(500), nullable=True)
    relation: Mapped[str] = mapped_column(String(120), nullable=False, default="challenges")
    visibility: Mapped[str] = mapped_column(String(30), nullable=False, default="internal")
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)

class ResearchAlternativeHypothesisRecord(Base):
    __tablename__ = "research_alternative_hypotheses_v294"
    __table_args__ = (UniqueConstraint("hypothesis_key", name="uq_research_alternative_hypothesis_v294_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    hypothesis_key: Mapped[str] = mapped_column(String(180), nullable=False)
    challenge_id: Mapped[str] = mapped_column(ForeignKey("research_validation_challenges_v294.id", ondelete="CASCADE"), nullable=False)
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    existing_hypothesis_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    statement: Mapped[str] = mapped_column(Text, nullable=False)
    rationale: Mapped[str | None] = mapped_column(Text, nullable=True)
    distinguishing_evidence_json: Mapped[list] = mapped_column(JSON, default=list)
    status: Mapped[str] = mapped_column(String(80), nullable=False, default="declared")
    visibility: Mapped[str] = mapped_column(String(30), nullable=False, default="internal")
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
...
class ResearchContradictionTestRecord(Base):
    __tablename__ = "research_contradiction_tests_v294"
    __table_args__ = (UniqueConstraint("test_key", name="uq_research_contradiction_test_v294_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    test_key: Mapped[str] = mapped_column(String(180), nullable=False)
    challenge_id: Mapped[str] = mapped_column(ForeignKey("research_validation_challenges_v294.id", ondelete="CASCADE"), nullable=False)
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    assertion_a_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    assertion_b_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    test_method_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    result_status: Mapped[str] = mapped_column(String(100), nullable=False, default="recorded")
    evidence_json: Mapped[list] = mapped_column(JSON, default=list)
    notes: Mapped[str | None] = mapped_column(Text, nullable=True)
    visibility: Mapped[str] = mapped_column(String(30), nullable=False, default="internal")
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
...
class ResearchCounterevidenceRecord(Base):
    __tablename__ = "research_counterevidence_v294"
    __table_args__ = (UniqueConstraint("counterevidence_key", name="uq_research_counterevidence_v294_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    counterevidence_key: Mapped[str] = mapped_column(String(180), nullable=False)
    challenge_id: Mapped[str] = mapped_column(ForeignKey("research_validation_challenges_v294.id", ondelete="CASCADE"), nullable=False)
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    evidence_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    challenges_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    relationship: Mapped[str] = mapped_column(String(100), nullable=False, default="counterevidence")
    statement: Mapped[str | None] = mapped_column(Text, nullable=True)
    visibility: Mapped[str] = mapped_column(String(30), nullable=False, default="internal")
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
...
class ResearchSensitivityCheckRecord(Base):
    __tablename__ = "research_sensitivity_checks_v294"
    __table_args__ = (UniqueConstraint("check_key", name="uq_research_sensitivity_check_v294_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    check_key: Mapped[str] = mapped_column(String(180), nullable=False)
    challenge_id: Mapped[str] = mapped_column(ForeignKey("research_validation_challenges_v294.id", ondelete="CASCADE"), nullable=False)
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    target_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    factor_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    method_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    execution_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    result_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    outcome: Mapped[str] = mapped_column(String(100), nullable=False, default="recorded")
    evidence_json: Mapped[list] = mapped_column(JSON, default=list)
    visibility: Mapped[str] = mapped_column(String(30), nullable=False, default="internal")
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
...
class ResearchRobustnessCheckRecord(Base):
    __tablename__ = "research_robustness_checks_v294"
    __table_args__ = (UniqueConstraint("check_key", name="uq_research_robustness_check_v294_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    check_key: Mapped[str] = mapped_column(String(180), nullable=False)
    challenge_id: Mapped[str] = mapped_column(ForeignKey("research_validation_challenges_v294.id", ondelete="CASCADE"), nullable=False)
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
...
class ResearchReplicationAttemptV294Record(Base):
    __tablename__ = "research_replication_attempts_v294"
    __table_args__ = (UniqueConstraint("attempt_key", name="uq_research_replication_attempt_v294_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    attempt_key: Mapped[str] = mapped_column(String(180), nullable=False)
    challenge_id: Mapped[str] = mapped_column(ForeignKey("research_validation_challenges_v294.id", ondelete="CASCADE"), nullable=False)
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    target_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    protocol_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    execution_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    package_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    result_status: Mapped[str] = mapped_column(String(100), nullable=False, default="recorded")
    evidence_json: Mapped[list] = mapped_column(JSON, default=list)
    visibility: Mapped[str] = mapped_column(String(30), nullable=False, default="internal")
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
...
class ResearchReviewerChallengeRecord(Base):
    __tablename__ = "research_reviewer_challenges_v294"
    __table_args__ = (UniqueConstraint("review_challenge_key", name="uq_research_reviewer_challenge_v294_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    review_challenge_key: Mapped[str] = mapped_column(String(180), nullable=False)
    challenge_id: Mapped[str] = mapped_column(ForeignKey("research_validation_challenges_v294.id", ondelete="CASCADE"), nullable=False)
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    reviewer_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    review_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    target_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    challenge_statement: Mapped[str] = mapped_column(Text, nullable=False)
    severity_label: Mapped[str | None] = mapped_column(String(80), nullable=True)
    status: Mapped[str] = mapped_column(String(80), nullable=False, default="open")
    visibility: Mapped[str] = mapped_column(String(30), nullable=False, default="internal")
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
...
class ResearchChallengeResponseRecord(Base):
    __tablename__ = "research_challenge_responses_v294"
    __table_args__ = (UniqueConstraint("response_key", name="uq_research_challenge_response_v294_key"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    response_key: Mapped[str] = mapped_column(String(180), nullable=False)
    challenge_id: Mapped[str] = mapped_column(ForeignKey("research_validation_challenges_v294.id", ondelete="CASCADE"), nullable=False)
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    responder_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    response_type: Mapped[str] = mapped_column(String(120), nullable=False, default="response")
    statement: Mapped[str] = mapped_column(Text, nullable=False)
    evidence_json: Mapped[list] = mapped_column(JSON, default=list)
    revised_object_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    status: Mapped[str] = mapped_column(String(80), nullable=False, default="recorded")
    visibility: Mapped[str] = mapped_column(String(30), nullable=False, default="internal")
    provenance_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_by: Mapped[str] = mapped_column(String(255), nullable=False, default="operator")
...
class ResearchValidationRevisionRecord(Base):
    __tablename__ = "research_validation_revisions_v294"
    __table_args__ = (UniqueConstraint("project_ref", "revision", name="uq_research_validation_revision_v294"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    revision: Mapped[int] = mapped_column(Integer, nullable=False)
    challenge_id: Mapped[str | None] = mapped_column(ForeignKey("research_validation_challenges_v294.id", ondelete="SET NULL"), nullable=True)
    change_type: Mapped[str] = mapped_column(String(120), nullable=False)
    prior_state_json: Mapped[dict] = mapped_column(JSON, default=dict)
    revised_state_json: Mapped[dict] = mapped_column(JSON, default=dict)
    change_summary: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class ResearchValidationSnapshotRecord(Base):
    __tablename__ = "research_validation_snapshots_v294"
    __table_args__ = (UniqueConstraint("project_ref", "revision", name="uq_research_validation_snapshot_v294_revision"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    project_ref: Mapped[str] = mapped_column(String(1000), nullable=False)
    revision: Mapped[int] = mapped_column(Integer, nullable=False)
    content_hash: Mapped[str] = mapped_column(String(64), nullable=False)

...
from ..services.gretl_hansl_runtime import (
    CONTRACT_VERSION,
    CORE_RELEASE,
...
router = APIRouter(prefix="/api/v1/gretl-runtime", tags=["gretl-runtime"])
public_router = APIRouter(prefix="/public/v1/gretl-runtime", tags=["public-gretl-runtime"])
...
def validate_request(body: GretlExecutionRequest):
    return {
        "ok": True,
        "release": CORE_RELEASE,
        "contract": CONTRACT_VERSION,
...
def validate_bundle(body: GretlRuntimeBundle):
    return {
        "ok": True,
        "release": CORE_RELEASE,
        "contract": CONTRACT_VERSION,
...
def scientific_artifact(body: GretlRuntimeBundle):
    return {
        "ok": True,
        "release": CORE_RELEASE,
        "contract": CONTRACT_VERSION,
        "scientific_artifact": to_scientific_gretl_artifact(body),
    }

...
from ..services.octave_runtime import (
    CONTRACT_VERSION, CORE_RELEASE, OctaveExecutionRequest, OctaveRuntimeBundle,
    contract_document, reference_environment_package, reference_runtime_bundle,
    reference_security_policy, to_scientific_octave_artifact,
)

router = APIRouter(prefix="/api/v1/octave-runtime", tags=["octave-runtime"])
public_router = APIRouter(prefix="/public/v1/octave-runtime", tags=["public-octave-runtime"])
...
def validate_request(body: OctaveExecutionRequest):
    return {"ok": True, "release": CORE_RELEASE, "contract": CONTRACT_VERSION,
            "request_fingerprint_sha256": body.fingerprint()}

@router.post("/validate-bundle")
def validate_bundle(body: OctaveRuntimeBundle):
    return {"ok": True, "release": CORE_RELEASE, "contract": CONTRACT_VERSION,
            "bundle_fingerprint_sha256": body.fingerprint()}

@router.post("/scientific-artifact")
def scientific_artifact(body: OctaveRuntimeBundle):
    return {"ok": True, "release": CORE_RELEASE, "contract": CONTRACT_VERSION,
            "scientific_artifact": to_scientific_octave_artifact(body)}

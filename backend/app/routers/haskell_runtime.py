...
from ..services.haskell_runtime import (
    CONTRACT_VERSION,
    CORE_RELEASE,
    HaskellExecutionRequest,
    HaskellRuntimeBundle,
    contract_document,
    reference_environment_package,
    reference_runtime_bundle,
    reference_security_policy,
    to_scientific_haskell_artifact,
)

router = APIRouter(prefix="/api/v1/haskell-runtime", tags=["haskell-runtime"])
public_router = APIRouter(prefix="/public/v1/haskell-runtime", tags=["public-haskell-runtime"])
...
def validate_request(body: HaskellExecutionRequest):
    return {
        "ok": True,
        "release": CORE_RELEASE,
        "contract": CONTRACT_VERSION,
...
def validate_bundle(body: HaskellRuntimeBundle):
    return {
        "ok": True,
        "release": CORE_RELEASE,
        "contract": CONTRACT_VERSION,
...
def scientific_artifact(body: HaskellRuntimeBundle):
    return {
        "ok": True,
        "release": CORE_RELEASE,
        "contract": CONTRACT_VERSION,
        "scientific_artifact": to_scientific_haskell_artifact(body),
    }

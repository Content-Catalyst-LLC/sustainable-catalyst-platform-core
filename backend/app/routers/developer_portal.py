...
<p class="lede">Build public-interest research, sustainability, evidence, knowledge-graph, decision-support, and cross-product gateway integrations on one versioned API. The unified public surface exposes reviewed registry, graph, evidence, and ledger records without exposing Platform Core administrative controls.</p>
<div class="actions">
...
<tr><td>Capacity governance</td><td><code>GET /api/v1/capacity/status</code></td><td><code>data:read</code></td></tr>
...
def python_sdk(request: Request):
    _require_portal(request)
    path = SDK_ROOT / "downloads" / "sc-platform-core-public-python-v2.37.0.2.zip"
    if not path.exists():
...
def javascript_sdk(request: Request):
    ...
    path = (
        SDK_ROOT
        / "downloads"
        / "sc-platform-core-public-javascript-v2.37.0.2.zip"
    )
    if not path.exists():

...
def readiness(db: Session, settings) -> dict:
    return {"enabled":settings.production_certification_enabled,"schema_head":SCHEMA_HEAD,"recovery_checkpoint_enabled":settings.recovery_checkpoint_enabled,"zero_pending_migrations_required":settings.certification_require_zero_pending_migrations,"valid_audit_chain_required":settings.certification_require_valid_audit_chain,"gateway_release_ready_required":settings.certification_require_gateway_release_ready,"capacity_ready_required":settings.certification_require_capacity_ready,"external_provider_health_release_blocking":False,"database_backup_embedded":False}
...
def run_certification(session: Session, database, settings, gateway_snapshot: dict | None=None):
    ...
    core={"release":settings.version,"state":state,"migration_head":mig['schema_head'],"pending_migrations":mig['pending'],"checks":checks,"blockers":blockers,"gateway":{"release_ready":gateway.get('release_ready',False),"required_blockers":gateway.get('required_blockers',[])},"resource_governance":capacity,"recovery":recovery}

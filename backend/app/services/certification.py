...
def readiness(db: Session, settings) -> dict:
    return {"enabled":settings.production_certification_enabled,"schema_head":SCHEMA_HEAD,"recovery_checkpoint_enabled":settings.recovery_checkpoint_enabled,"zero_pending_migrations_required":settings.certification_require_zero_pending_migrations,"valid_audit_chain_required":settings.certification_require_valid_audit_chain,"gateway_release_ready_required":settings.certification_require_gateway_release_ready,"capacity_ready_required":settings.certification_require_capacity_ready,"credential_lifecycle_ready_required":settings.certification_require_credential_lifecycle_ready,"workload_governance_ready_required":settings.certification_require_workload_governance_ready,"scientific_object_storage_ready_required":settings.certification_require_scientific_object_storage_ready,"external_provider_health_release_blocking":False,"database_backup_embedded":False}
...
def _table_inventory(session: Session):
    ...
    counts={}
    critical={"schema_migrations","entities","evidence_records","live_data_sources","operational_facilities","humanitarian_conditions","cross_product_exchange_packages","governance_audit_events","scale_processing_jobs","production_certification_runs","recovery_readiness_checkpoints","observability_metric_samples","service_level_objectives","production_deployment_markers","operations_incidents","operations_incident_events","change_control_records","rollback_coordination_records","backup_artifact_records","disaster_recovery_objectives","restore_rehearsal_records","region_service_status_records","failover_group_records","failover_assessment_records","data_lifecycle_policy_records","preservation_archive_records","federation_node_records","federation_trust_relationships","federation_exchange_manifests","federation_remote_references","capacity_resource_profiles","capacity_observations","capacity_budgets","capacity_forecast_records","capacity_governance_decisions","credential_registry_records","credential_key_versions","credential_rotation_records","credential_lifecycle_events","credential_use_events","workload_class_records","distributed_quota_policies","distributed_quota_usage_buckets","workload_admission_decisions","workload_admission_leases","scientific_storage_backends","scientific_stored_objects","scientific_processing_adapters","scientific_processing_runs"}
    for name in names:
        if name in critical:
...
def run_certification(session: Session, database, settings, gateway_snapshot: dict | None=None):
    ...
    from .federation import certification_snapshot as federation_certification_snapshot
    federation=federation_certification_snapshot(session,settings) if settings.federation_trusted_node_exchange_enabled else {"state":"disabled","federation_ready":False,"signature_mismatches":0,"missing_runtime_secrets":0,"automatic_truth_promotion":False}
    from .capacity import certification_snapshot as capacity_certification_snapshot
    capacity=capacity_certification_snapshot(session,settings) if settings.capacity_resource_governance_enabled else {"state":"disabled","capacity_ready":False,"critical_profiles":0,"forecast_covered_profiles":0,"resource_profiles":0,"automatic_scaling":False,"automatic_infrastructure_purchase":False,"hard_admission_control":False}
    from .credentials import certification_snapshot as credential_certification_snapshot
    credential_lifecycle=credential_certification_snapshot(session,settings) if settings.credential_key_lifecycle_enabled else {"state":"disabled","credential_lifecycle_ready":False,"secret_values_persisted":False,"automatic_key_rotation":False}
    from .workload_governance import certification_snapshot as workload_certification_snapshot
    workload_governance=workload_certification_snapshot(session,settings) if settings.workload_governance_enabled else {"state":"disabled","workload_governance_ready":False,"hard_admission_control":False,"shared_state":False}
    from .scientific_objects import certification_snapshot as scientific_object_certification_snapshot
    scientific_objects=scientific_object_certification_snapshot(session,settings) if settings.scientific_object_storage_enabled else {"state":"disabled","scientific_object_storage_ready":False,"local_storage_ready":False,"processing_adapters":0,"executable_adapters":0,"arbitrary_code_execution":False,"credential_values_persisted":False,"external_fetch_by_core":False}
    checks={"database_roundtrip":db_roundtrip,"migration_head_matches":mig['head_matches'],"zero_pending_migrations":mig['zero_pending'],"governance_audit_chain_valid":bool(audit.get('valid')),"recovery_checkpoint_valid":bool(checkpoint_check.get('valid')) if settings.recovery_checkpoint_enabled else True,"gateway_release_ready":bool(gateway.get('release_ready',False)),"recent_verified_backup":bool(continuity.get('recent_verified_backup')),"recent_restore_rehearsal":bool(continuity.get('recent_restore_rehearsal')),"rpo_met":bool(continuity.get('rpo_met')),"rto_met":bool(continuity.get('rto_met')),"multi_region_ready":bool(resilience.get('multi_region_ready')),"preservation_ready":bool(preservation.get('preservation_ready')),"federation_ready":bool(federation.get('federation_ready')),"capacity_ready":bool(capacity.get('capacity_ready')),"capacity_critical_profiles":int(capacity.get('critical_profiles',0)),"credential_lifecycle_ready":bool(credential_lifecycle.get('credential_lifecycle_ready')),"credential_persisted_secret_values":int(credential_lifecycle.get('persisted_secret_values',0)),"workload_governance_ready":bool(workload_governance.get('workload_governance_ready')),"hard_admission_control":bool(workload_governance.get('hard_admission_control')),"scientific_object_storage_ready":bool(scientific_objects.get('scientific_object_storage_ready')),"external_provider_health_release_blocking":False}
    blockers=[]
    ...
    if settings.certification_require_federation_ready and not federation.get('federation_ready'): blockers.append("federated_core_trusted_node_exchange")
    if settings.certification_require_capacity_ready and not capacity.get('capacity_ready'): blockers.append("capacity_resource_governance")
    if settings.certification_require_credential_lifecycle_ready and not credential_lifecycle.get('credential_lifecycle_ready'): blockers.append("credential_key_lifecycle")
    if settings.certification_require_workload_governance_ready and not workload_governance.get('workload_governance_ready'): blockers.append("distributed_quota_admission_control")
    if settings.certification_require_scientific_object_storage_ready and not scientific_objects.get('scientific_object_storage_ready'): blockers.append("scientific_object_storage_processing_adapter_fabric")
    state="certified" if not blockers else "blocked"
    recovery={"checkpoint_id":checkpoint.id if checkpoint else None,"checkpoint_valid":checkpoint_check.get('valid',False),"database_backup_embedded":False,"external_backup_required_for_full_restore":True,"disaster_recovery":continuity,"multi_region_resilience":resilience,"data_lifecycle_preservation":preservation,"federated_core_trusted_node_exchange":federation}
    core={"release":settings.version,"state":state,"migration_head":mig['schema_head'],"pending_migrations":mig['pending'],"checks":checks,"blockers":blockers,"gateway":{"release_ready":gateway.get('release_ready',False),"required_blockers":gateway.get('required_blockers',[])},"resource_governance":capacity,"credential_key_lifecycle":credential_lifecycle,"workload_governance":workload_governance,"scientific_object_storage":scientific_objects,"recovery":recovery}

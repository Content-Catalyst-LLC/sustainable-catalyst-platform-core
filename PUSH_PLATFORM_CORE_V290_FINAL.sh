...
PACKAGE_ZIP="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.9.0-repository.zip}"
WORKDIR="${2:-${HOME}/Downloads/sustainable-catalyst-platform-core-repo}"
EXTRACT_DIR="${TMPDIR:-/tmp}/sc-platform-core-v290-extracted"

printf '%s\n' \
  "============================================================" \
  "PLATFORM CORE v2.9.0 — STREAMING, ALERTS & RELIABILITY" \
  "============================================================" \
  "Package:    ${PACKAGE_ZIP}" \
  "Repository: ${WORKDIR}" \
  "Remote:     ${REMOTE}" \
...
SOURCE_ROOT="$(find "$EXTRACT_DIR" -maxdepth 1 -type d -name 'sustainable-catalyst-platform-core-v2.9.0' | head -1)"
[ -n "$SOURCE_ROOT" ] && [ -f "$SOURCE_ROOT/backend/app/main.py" ] || { echo "ERROR: Expected v2.9.0 repository root was not found."; exit 1; }

if [ -d "$WORKDIR/.git" ]; then
  cd "$WORKDIR"
  git remote set-url origin "$REMOTE"
  if ! git diff --quiet || ! git diff --cached --quiet || [ -n "$(git ls-files --others --exclude-standard)" ]; then
    git stash push -u -m "Automatic safety stash before Platform Core v2.9.0" >/dev/null || true
  fi
...
"$PYTHON_BIN" scripts/validate_v290_release.py
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
root=Path('.')
m=json.loads((root/'BUILD_MANIFEST.json').read_text())
assert m.get('release')=='2.9.0', m.get('release')
...
SMOKE_DB="$WORKDIR/backend/platform_core_v290_push_smoke.db"
rm -f "$SMOKE_DB"
...
mkdir -p dist
(cd wordpress-plugin && zip -qr ../dist/sustainable-catalyst-platform-core-plugin-v2.9.0.zip sustainable-catalyst-platform-core)
cp backend/public_sdk/downloads/sc-platform-core-public-python-v2.9.0.zip dist/
cp backend/public_sdk/downloads/sc-platform-core-public-javascript-v2.9.0.zip dist/
cp backend/public_sdk/postman/Sustainable_Catalyst_Public_API_v1.postman_collection.json dist/

git add -A
if git diff --cached --quiet; then echo "No repository changes need to be committed."; else git commit -m "Build Platform Core v2.9.0 streaming alerts and source reliability"; fi
git push --set-upstream origin "$BRANCH"

echo "PASS - Platform Core v2.9.0 pushed successfully"
if [ -n "${SC_CORE_PUBLIC_BASE_URL:-}" ]; then
  curl -fsS "${SC_CORE_PUBLIC_BASE_URL%/}/health" >/dev/null
  curl -fsS "${SC_CORE_PUBLIC_BASE_URL%/}/ready" | "$PYTHON_BIN" -c 'import json,sys; d=json.load(sys.stdin); assert d.get("ok") is True, d; assert d.get("streaming_alerts_source_reliability")=="ready", d; print("PASS - live /ready")'
  curl -fsS -H "X-SC-API-Key: ${SC_CORE_WRITE_API_KEY:-}" "${SC_CORE_PUBLIC_BASE_URL%/}/v1/reliability/readiness" | "$PYTHON_BIN" -c 'import json,sys; d=json.load(sys.stdin); assert d.get("release")=="2.9.0", d; print("PASS - live reliability readiness")'
else

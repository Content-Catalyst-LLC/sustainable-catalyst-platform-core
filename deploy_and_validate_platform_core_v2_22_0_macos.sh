#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.22.0 validation stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR
BUNDLE_INPUT="${1:-sustainable-catalyst-platform-core-v2.22.0-release-bundle.zip}"
BUNDLE="$(cd "$(dirname "$BUNDLE_INPUT")" && pwd)/$(basename "$BUNDLE_INPUT")"
WORK="${TMPDIR:-/tmp}/sc-platform-core-v2220-bundle"
...
PLUGIN_ZIP="sustainable-catalyst-platform-core-v2.22.0-wordpress-plugin.zip"
...
ROOT="repo-extract/sustainable-catalyst-platform-core-v2.22.0"
...
m=json.loads(pathlib.Path('BUILD_MANIFEST.json').read_text()); assert m['release']=='2.22.0'
for i in m['files']:
...
if [ "${SC_CORE_BUNDLE_ONLY:-0}" = "1" ]; then echo "PASS - v2.22.0 bundle-only verification complete"; exit 0; fi
...
SMOKE_DB="$WORK/platform_core_v2220_smoke.db"; rm -f "$SMOKE_DB"
...
rm -f "$SMOKE_DB"
echo "PASS - v2.22.0 deterministic validation complete"
if [ "${SC_CORE_VALIDATE_ONLY:-0}" = "1" ]; then exit 0; fi
cd "$WORK"; chmod +x PUSH_PLATFORM_CORE_V2220_FINAL.sh; ./PUSH_PLATFORM_CORE_V2220_FINAL.sh "$WORK/$REPO_ZIP"

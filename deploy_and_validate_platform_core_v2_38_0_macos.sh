#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.38.0 validation stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR
BUNDLE="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.38.0-release-bundle.zip}"
select_python(){ local c; for c in python3.12 python3.11 python3.10 python3; do if command -v "$c" >/dev/null 2>&1 && "$c" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3,10) else 1)' >/dev/null 2>&1; then command -v "$c"; return 0; fi; done; echo "ERROR: Platform Core v2.38.0 requires Python 3.10+; Python 3.12 is preferred." >&2; return 1; }
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/sc-platform-core-v2380-bundle.XXXXXX")"; trap 'rm -rf "$WORK"' EXIT
...
REPO_ZIP="sustainable-catalyst-platform-core-v2.38.0-repository.zip"; PLUGIN_ZIP="sustainable-catalyst-platform-core-v2.38.0-wordpress-plugin.zip"
unzip -t "$REPO_ZIP" >/dev/null; unzip -t "$PLUGIN_ZIP" >/dev/null
mkdir -p "$WORK/repo-extract"; unzip -q "$REPO_ZIP" -d "$WORK/repo-extract"
ROOT="$WORK/repo-extract/sustainable-catalyst-platform-core-v2.38.0"; [ -f "$ROOT/backend/app/main.py" ] || { echo "ERROR: repository root missing"; exit 1; }
cd "$ROOT"
# MUST remain dependency-free: no backend venv/pip exists yet.
"$PYTHON_BIN" -S scripts/validate_v2380_release.py
"$PYTHON_BIN" scripts/scan_push_safe_secrets.py .
"$PYTHON_BIN" - <<'PY2'
import hashlib,json,pathlib
m=json.loads(pathlib.Path('BUILD_MANIFEST.json').read_text()); assert m['release']=='2.38.0',m
for item in m['files']:
...
bash -n PUSH_PLATFORM_CORE_V2380_FINAL.sh; bash -n deploy_and_validate_platform_core_v2_38_0_macos.sh; bash -n DEPLOY_PLATFORM_CORE_V2380_CONTABO.sh
if [ "${SC_CORE_BUNDLE_ONLY:-0}" = "1" ]; then echo "PASS - v2.38.0 bundle-only verification complete without backend dependencies"; exit 0; fi
...
SMOKE_DB="$WORK/platform_core_v2380_smoke.db"; rm -f "$SMOKE_DB"
...
backend/.venv/bin/python backend/scripts/migrate.py >/tmp/sc-core-v2380-migrate.json
PYTHONPATH=backend backend/.venv/bin/python backend/scripts/validate_spatial_temporal.py
backend/.venv/bin/python - <<'PY2'
import json
m=json.load(open('/tmp/sc-core-v2380-migrate.json')); assert '0042' in m['applied'] and m['pending']==[],m
print('PASS - migration 0042 applied with pending=[]')
PY2
...
echo "PASS - v2.38.0 release-critical validation complete"
if [ "${SC_CORE_VALIDATE_ONLY:-0}" = "1" ]; then exit 0; fi
cd "$WORK/bundle"; chmod +x PUSH_PLATFORM_CORE_V2380_FINAL.sh; PYTHON_BIN="$PYTHON_BIN" ./PUSH_PLATFORM_CORE_V2380_FINAL.sh "$WORK/bundle/$REPO_ZIP"

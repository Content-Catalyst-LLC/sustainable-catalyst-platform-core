#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.23.1 promotion stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR

ORG="Content-Catalyst-LLC"
REPO="sustainable-catalyst-platform-core"
BRANCH="main"
REMOTE="git@github.com:${ORG}/${REPO}.git"
PACKAGE_ZIP="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.23.1-repository.zip}"
WORKDIR="${2:-${HOME}/Downloads/sustainable-catalyst-platform-core-repo}"
EXTRACT_DIR="${TMPDIR:-/tmp}/sc-platform-core-v2231-extracted"

printf '%s\n' \
  "============================================================" \
  "PLATFORM CORE v2.23.1 — CAPABILITY METADATA / RELEASE LINEAGE" \
  "============================================================" \
  "Package:    ${PACKAGE_ZIP}" \
  "Repository: ${WORKDIR}" \
  "Remote:     ${REMOTE}" ""

for c in git unzip rsync grep find zip shasum php bash node curl; do
  command -v "$c" >/dev/null 2>&1 || { echo "ERROR: Required command not found: $c"; exit 1; }
done
...
SOURCE_ROOT="$(find "$EXTRACT_DIR" -maxdepth 1 -type d -name 'sustainable-catalyst-platform-core-v2.23.1' | head -1)"
[ -n "$SOURCE_ROOT" ] && [ -f "$SOURCE_ROOT/backend/app/main.py" ] || { echo "ERROR: Expected v2.23.1 repository root was not found."; exit 1; }

if [ -d "$WORKDIR/.git" ]; then
  cd "$WORKDIR"
  git remote set-url origin "$REMOTE"
  if ! git diff --quiet || ! git diff --cached --quiet || [ -n "$(git ls-files --others --exclude-standard)" ]; then
    git stash push -u -m "Automatic safety stash before Platform Core v2.23.1" >/dev/null || true
  fi
...
rsync -a --delete \
  --exclude='.git/' --exclude='.venv/' --exclude='.pytest_cache/' --exclude='__pycache__/' --exclude='*.db' \
  "$SOURCE_ROOT/" "$WORKDIR/"
cd "$WORKDIR"
...
import hashlib, json
m = json.loads(Path('BUILD_MANIFEST.json').read_text())
assert m.get('release') == '2.23.1', m.get('release')
for item in m['files']:
    p = Path(item['path'])
...
SECRET_HITS="$({ grep -RInE '(sk-proj-[A-Za-z0-9_-]{20,}|sk-[A-Za-z0-9_-]{30,}|AIza[0-9A-Za-z_-]{20,}|gh[opusr]_[A-Za-z0-9_]{20,}|Authorization:[[:space:]]*Bearer[[:space:]]+[A-Za-z0-9._-]{24,}|X-SC-Service-Token:[[:space:]]*[A-Za-z0-9._-]{20,}|SC_CORE_FEDERATION_TRUST_SECRETS_JSON=[^#[:space:]]+)' . \
  --exclude-dir=.git --exclude-dir=.venv --exclude-dir=.pytest_cache --exclude-dir=__pycache__ \
  --exclude='*.zip' --exclude='*.pyc' --exclude='.env.example' --exclude='platform-core-v2230.env.example' --exclude='platform-core-v2231.env.example' \
  --exclude='PUSH_PLATFORM_CORE_V*_FINAL*.sh' || true; } | grep -viE '(placeholder|replace[-_ ]?me|change[-_ ]?me|DEMO_KEY|test-secret|secret-token|shared-federation-secret|hidden)' || true)"
...
SMOKE_DB="$WORKDIR/backend/platform_core_v2231_push_smoke.db"
rm -f "$SMOKE_DB"
...
mkdir -p dist
(cd wordpress-plugin && zip -qr ../dist/sustainable-catalyst-platform-core-plugin-v2.23.1.zip sustainable-catalyst-platform-core)
cp backend/public_sdk/downloads/sc-platform-core-public-python-v2.23.1.zip dist/
cp backend/public_sdk/downloads/sc-platform-core-public-javascript-v2.23.1.zip dist/

git add -A
if git diff --cached --quiet; then
  echo "No repository changes need to be committed."
else
  git commit -m "Build Platform Core v2.23.1 capability metadata release-lineage repair"
fi
git push --set-upstream origin "$BRANCH"
echo "PASS - Platform Core v2.23.1 pushed successfully"

if [ -n "${SC_CORE_PUBLIC_BASE_URL:-}" ]; then
  curl -fsS "${SC_CORE_PUBLIC_BASE_URL%/}/health" | "$PYTHON_BIN" -c 'import json,sys; d=json.load(sys.stdin); assert d.get("version")=="2.23.1",d; print("PASS - live /health")'
  curl -fsS "${SC_CORE_PUBLIC_BASE_URL%/}/v1/federation/readiness" | "$PYTHON_BIN" -c 'import json,sys; d=json.load(sys.stdin); assert d.get("release")=="2.23.1",d; assert d.get("migration_0026_applied") is True,d; print("PASS - live federation readiness")'
  curl -fsS "${SC_CORE_PUBLIC_BASE_URL%/}/v1/meta" | "$PYTHON_BIN" -c 'import json,sys; d=json.load(sys.stdin); caps=set(d.get("capabilities",[])); deferred=set(d.get("deferred_capabilities",[])); assert {"distributed_connector_workers","server_sent_live_data_events"} <= caps; assert not ({"distributed_connector_workers","server_sent_live_data_events"} & deferred); print("PASS - live capability truth")'
else

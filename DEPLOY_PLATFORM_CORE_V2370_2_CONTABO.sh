#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.37.0.2 Contabo deployment stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR
ROOT="/opt/sustainable-catalyst/core"; BACKUP_ROOT="/opt/sustainable-catalyst/backups"; COMPOSE="docker compose -f compose.yml -f compose.vps.yml"; STAMP="$(date +%Y%m%d-%H%M%S)"; BACKUP="${BACKUP_ROOT}/platform-core-v2.37.0.2-${STAMP}"
cd "$ROOT"
echo "============================================================"; echo " PLATFORM CORE v2.37.0.2 — BUNDLE VALIDATOR BOOTSTRAP REPAIR"; echo "============================================================"
...
$COMPOSE config >/tmp/sc-core-v23702-compose.yml; grep -q '/data/scientific-objects' /tmp/sc-core-v23702-compose.yml || { echo "STOP: scientific-object persistent mount missing"; exit 1; }
...
echo; echo "=== BACKUP CURRENT CORE + DATABASE ==="; mkdir -p "$BACKUP"; tar --exclude='.git' -czf "$BACKUP/core-before-v2.37.0.2.tar.gz" .; docker exec -e PGPASSWORD="$DB_PASS" sc-postgres pg_dump -h 127.0.0.1 -U "$DB_USER" -d "$DB_NAME" -Fc > "$BACKUP/platform-core-before-v2.37.0.2.dump"; ls -lh "$BACKUP"/*
echo; echo "=== FETCH EXACT v2.37.0.2 RELEASE ==="; git fetch origin --tags; git rev-parse v2.37.0.2 >/dev/null 2>&1 || { echo "STOP: v2.37.0.2 tag is not available from GitHub"; exit 1; }; git pull --ff-only origin main; echo "HEAD: $(git log -1 --oneline)"; git tag --points-at HEAD | grep -qx 'v2.37.0.2' || { echo "STOP: production HEAD is not exactly tagged v2.37.0.2"; exit 1; }
head_commit="$(git rev-parse HEAD)"; tag_commit="$(git rev-list -n 1 v2.37.0.2)"; [ "$head_commit" = "$tag_commit" ] || { echo "STOP: v2.37.0.2 does not resolve to production HEAD"; exit 1; }
python3 - <<'PY2'
import sys
...
PY2
echo; echo "=== BUILD CORE v2.37.0.2 ==="; $COMPOSE build core
echo; echo "=== APPLY / REPAIR MIGRATION 0041 ==="; $COMPOSE run --rm --no-deps core python scripts/migrate.py | tee /tmp/sc-core-v23702-migrations.json
...
for i in $(seq 1 40); do if curl -fsS http://127.0.0.1:8090/health >/tmp/sc-core-v23702-health.json 2>/dev/null; then break; fi; [ "$i" -lt 40 ] || { docker logs --tail 200 sc-core; exit 1; }; sleep 2; done
...
echo; echo "=== LIVE HEALTH ==="; python3 -m json.tool </tmp/sc-core-v23702-health.json
echo; echo "=== CAUSAL SYSTEMS READINESS ==="; curl -fsS http://127.0.0.1:8090/v1/causal-systems/readiness | tee /tmp/sc-core-v23702-causal.json | python3 -m json.tool
python3 - <<'PY2'
import json
h=json.load(open('/tmp/sc-core-v23702-health.json')); c=json.load(open('/tmp/sc-core-v23702-causal.json'))
assert h.get('ok') is True and h.get('version')=='2.37.0.2',h
assert h.get('causal_systems_explorer') is True,h
assert c.get('release')=='2.37.0.2' and c.get('migration_0041_applied') is True,c
...
print('PASS: Platform Core v2.37.0.2'); print('PASS: Causal Systems Explorer'); print('PASS: causal migration metadata/tag identity repair retained'); print('PASS: bundle validator bootstrap repair')
PY2
echo; echo "=== VERIFY MIGRATION STATE ==="; docker exec sc-core python scripts/migrate.py | tee /tmp/sc-core-v23702-migration-state.json
python3 - <<'PY2'
import json
m=json.load(open('/tmp/sc-core-v23702-migration-state.json')); assert '0041' in m.get('applied',[]) and m.get('pending')==[],m; print('PASS: migration 0041 applied and pending=[]')
PY2
echo; echo "=== VERIFY PERSISTENT SCIENTIFIC STORAGE MOUNT ==="; docker inspect sc-core --format '{{json .Mounts}}' | python3 -m json.tool; docker exec sc-core sh -lc 'test -d /data/scientific-objects; touch /data/scientific-objects/.v23702-write-test; rm /data/scientific-objects/.v23702-write-test; echo "PASS: persistent scientific-object storage writable"'
echo; echo "=== PUBLIC CADDY ROUTE ==="; curl -fsS https://core.sustainablecatalyst.com/health | python3 -c 'import json,sys;d=json.load(sys.stdin);assert d.get("version")=="2.37.0.2",d;print("PASS: public Core health 2.37.0.2")'; curl -fsS https://core.sustainablecatalyst.com/v1/causal-systems/readiness | python3 -c 'import json,sys;d=json.load(sys.stdin);assert d.get("release")=="2.37.0.2" and d.get("migration_0041_applied") is True,d;print("PASS: public Causal Systems readiness")'
echo; echo "=== FINAL CONTAINER ==="; docker ps --filter name=sc-core --format 'table {{.Names}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}'; echo; echo "Backup retained at: $BACKUP"; echo "============================================================"; echo " PLATFORM CORE v2.37.0.2 BACKEND DEPLOYMENT COMPLETE"; echo "============================================================"

#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.37.0.1 promotion stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR
PACKAGE_ZIP="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.37.0.1-repository.zip}"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-platform-core-v23701-push.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
unzip -q "$PACKAGE_ZIP" -d "$TMP"
SOURCE_ROOT="$TMP/sustainable-catalyst-platform-core-v2.37.0.1"
[ -f "$SOURCE_ROOT/backend/app/main.py" ] || { echo "ERROR: expected v2.37.0.1 source root missing"; exit 1; }
...
if git rev-parse -q --verify "refs/tags/$TAG" >/dev/null 2>&1 || git ls-remote --exit-code --tags origin "refs/tags/$TAG" >/dev/null 2>&1; then
  echo "STOP: tag v2.37.0.1 already exists; refusing to reuse or move a release tag."
  exit 1
fi
...
bash -n deploy_and_validate_platform_core_v2_37_0_1_macos.sh
bash -n DEPLOY_PLATFORM_CORE_V2370_1_CONTABO.sh
git add -A
if git diff --cached --quiet; then echo "No repository changes need to be committed."; else git commit -m "Repair Platform Core v2.37.0.1 causal migration metadata and tag identity"; fi
...
git tag -a "$TAG" -m "Platform Core v2.37.0.1 — Causal Migration Metadata Repair" "$head_commit"
tag_commit="$(git rev-list -n 1 "$TAG")"
[ "$tag_commit" = "$head_commit" ] || { echo "STOP: release tag does not resolve to promoted HEAD"; exit 1; }
git push origin "$TAG"
remote_tag_commit="$(git ls-remote origin "refs/tags/$TAG^{}" | awk '{print $1}')"
[ -n "$remote_tag_commit" ] || remote_tag_commit="$(git ls-remote origin "refs/tags/$TAG" | awk '{print $1}')"
...
echo "PASS - tag v2.37.0.1 resolves to exact promoted HEAD $head_commit"
echo "PASS - Platform Core v2.37.0.1 pushed successfully"

#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.94.0 promotion stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR
ARCHIVE="${1:?usage: $0 /path/to/sustainable-catalyst-platform-core-v2.94.0-repository.zip}"
...
SRC="$(find "$TMP" -maxdepth 1 -type d -name 'sustainable-catalyst-platform-core-v2.94.0' | head -1)"
[ -n "$SRC" ] || { echo "STOP: archive root not found"; exit 1; }
python3 -S "$SRC/scripts/validate_v2940_release.py"
echo "=== BOOTSTRAP V2.94 VALIDATION ENVIRONMENT ==="
VENV="$TMP/v2940-venv"; python3 -m venv "$VENV"; "$VENV/bin/python" -m pip install --disable-pip-version-check -q -r "$SRC/backend/requirements.txt"
PYTHONPATH="$SRC/backend" "$VENV/bin/python" "$SRC/backend/scripts/validate_research_validation_challenge.py"
...
python3 -S scripts/validate_v2940_release.py
PYTHONPATH="$REPO/backend" "$VENV/bin/python" backend/scripts/validate_research_validation_challenge.py
...
bash -n DEPLOY_PLATFORM_CORE_V2940_CONTABO.sh
git add -A
if git diff --cached --quiet; then echo "No repository changes need to be committed."; else git commit -m "Build Platform Core v2.94.0 Research Validation & Challenge Engine"; fi
...
git tag -a "$TAG" -m "Platform Core v2.94.0 Research Validation & Challenge Engine"; git push origin "$TAG"
...
echo "PASS - tag v2.94.0 resolves to exact promoted HEAD $head_commit"
echo "PASS - Platform Core v2.94.0 pushed successfully"

#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.79.0 promotion stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR

PACKAGE_ZIP="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.79.0-repository.zip}"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-platform-core-v2790-push.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$PACKAGE_ZIP" -d "$TMP"
SOURCE_ROOT="$TMP/sustainable-catalyst-platform-core-v2.79.0"
[ -f "$SOURCE_ROOT/backend/app/main.py" ] || { echo "ERROR: expected v2.79.0 source root missing"; exit 1; }
...
if git diff --cached --quiet; then
  ...
  git commit -m "Build Platform Core v2.79.0 Research Argument & Evidentiary Synthesis Engine"
fi
...
git tag -a "$TAG" -m "Platform Core v2.79.0 Research Argument & Evidentiary Synthesis Engine"
git push origin "$TAG"
...
echo "PASS - tag v2.79.0 resolves to exact promoted HEAD $head_commit"
echo "PASS - Platform Core v2.79.0 pushed successfully"

#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.36.1.2 validation stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR
BUNDLE="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.36.1.2-release-bundle.zip}"
select_python(){ local c; for c in python3.12 python3.11 python3.10 python3; do if command -v "$c" >/dev/null 2>&1 && "$c" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3,10) else 1)' >/dev/null 2>&1; then command -v "$c"; return 0; fi; done; echo "ERROR: Platform Core v2.36.1.2 requires Python 3.10+; Python 3.12 is preferred." >&2; return 1; }
...
bash -n PUSH_PLATFORM_CORE_V2361_2_FINAL.sh
bash -n deploy_and_validate_platform_core_v2_36_1_2_macos.sh
bash -n DEPLOY_PLATFORM_CORE_V2361_2_CONTABO.sh
if [ "${SC_CORE_BUNDLE_ONLY:-0}" = "1" ]; then
  echo "PASS - v2.36.1.2 bundle-only verification complete"
  exit 0
fi
...
SMOKE_DB="$WORK/platform_core_v2361_2_smoke.db"
rm -f "$SMOKE_DB"
...
backend/.venv/bin/python backend/scripts/migrate.py >/tmp/sc-core-v2361-2-migrate.json
backend/.venv/bin/python backend/scripts/validate_uncertainty_compute.py
backend/.venv/bin/python - <<'PY'
import json
m=json.load(open('/tmp/sc-core-v2361-2-migrate.json'))
assert '0040' in m['applied'] and m['pending']==[],m
print('PASS - migration 0040 applied with pending=[]')
PY
...
echo "PASS - v2.36.1.2 release-critical validation complete"
if [ "${SC_CORE_VALIDATE_ONLY:-0}" = "1" ]; then exit 0; fi
cd "$WORK/bundle"
chmod +x PUSH_PLATFORM_CORE_V2361_2_FINAL.sh
./PUSH_PLATFORM_CORE_V2361_2_FINAL.sh "$WORK/bundle/$REPO_ZIP"

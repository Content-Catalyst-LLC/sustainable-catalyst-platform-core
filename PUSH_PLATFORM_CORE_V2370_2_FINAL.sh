#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.37.0.2 promotion stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR
PACKAGE_ZIP="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.37.0.2-repository.zip}"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-platform-core-v23702-push.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
unzip -q "$PACKAGE_ZIP" -d "$TMP"
SOURCE_ROOT="$TMP/sustainable-catalyst-platform-core-v2.37.0.2"
[ -f "$SOURCE_ROOT/backend/app/main.py" ] || { echo "ERROR: expected v2.37.0.2 source root missing"; exit 1; }
...
if git rev-parse -q --verify "refs/tags/$TAG" >/dev/null 2>&1 || git ls-remote --exit-code --tags origin "refs/tags/$TAG" >/dev/null 2>&1; then
  echo "STOP: tag v2.37.0.2 already exists; refusing to reuse or move a release tag."
  exit 1
fi
...
"$PYTHON_BIN" scripts/validate_v2370_2_release.py
...
bash -n deploy_and_validate_platform_core_v2_37_0_2_macos.sh
bash -n DEPLOY_PLATFORM_CORE_V2370_2_CONTABO.sh
git add -A
if git diff --cached --quiet; then echo "No repository changes need to be committed."; else git commit -m "Repair Platform Core v2.37.0.2 bundle validator bootstrap"; fi
...
git tag -a "$TAG" -m "Platform Core v2.37.0.2 — Bundle Validator Bootstrap Repair" "$head_commit"
...
echo "PASS - tag v2.37.0.2 resolves to exact promoted HEAD $head_commit"
echo "PASS - Platform Core v2.37.0.2 pushed successfully"

#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.30.0 validation stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR

BUNDLE="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.30.0-release-bundle.zip}"
select_python() {
  local c
  ...
  echo "ERROR: Platform Core v2.30.0 requires Python 3.10+; Python 3.12 is preferred." >&2; return 1
}
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/sc-platform-core-v2300-bundle.XXXXXX")"; trap 'rm -rf "$WORK"' EXIT
unzip -q "$BUNDLE" -d "$WORK/bundle"; cd "$WORK/bundle"
[ -f SHA256SUMS.txt ] || { echo "ERROR: SHA256SUMS.txt missing"; exit 1; }
shasum -a 256 -c SHA256SUMS.txt
REPO_ZIP="sustainable-catalyst-platform-core-v2.30.0-repository.zip"
PLUGIN_ZIP="sustainable-catalyst-platform-core-v2.30.0-wordpress-plugin.zip"
unzip -t "$REPO_ZIP" >/dev/null; unzip -t "$PLUGIN_ZIP" >/dev/null
mkdir -p "$WORK/repo-extract"; unzip -q "$REPO_ZIP" -d "$WORK/repo-extract"
ROOT="$WORK/repo-extract/sustainable-catalyst-platform-core-v2.30.0"; [ -f "$ROOT/backend/app/main.py" ] || { echo "ERROR: repository root missing"; exit 1; }
cd "$ROOT"
...
m=json.loads(pathlib.Path('BUILD_MANIFEST.json').read_text()); assert m['release']=='2.30.0'
for item in m['files']:
...
bash -n PUSH_PLATFORM_CORE_V2300_FINAL.sh; bash -n deploy_and_validate_platform_core_v2_30_0_macos.sh; bash -n DEPLOY_PLATFORM_CORE_V2300_CONTABO.sh

if [ "${SC_CORE_BUNDLE_ONLY:-0}" = "1" ]; then echo "PASS - v2.30.0 bundle-only verification complete"; exit 0; fi
...
SMOKE_DB="$WORK/platform_core_v2300_smoke.db"; rm -f "$SMOKE_DB"
...
rm -f "$SMOKE_DB"; echo "PASS - v2.30.0 deterministic validation complete"
if [ "${SC_CORE_VALIDATE_ONLY:-0}" = "1" ]; then exit 0; fi
cd "$WORK/bundle"; chmod +x PUSH_PLATFORM_CORE_V2300_FINAL.sh; ./PUSH_PLATFORM_CORE_V2300_FINAL.sh "$WORK/bundle/$REPO_ZIP"

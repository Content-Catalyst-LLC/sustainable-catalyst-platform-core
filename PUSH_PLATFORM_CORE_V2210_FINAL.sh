...
PACKAGE_ZIP="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.21.0-repository.zip}"
WORKDIR="${2:-${HOME}/Downloads/sustainable-catalyst-platform-core-repo}"
EXTRACT_DIR="${TMPDIR:-/tmp}/sc-platform-core-v2210-extracted"
printf '%s\n' "============================================================" "PLATFORM CORE v2.21.0 — MULTI-REGION RESILIENCE & FAILOVER COORDINATION" "============================================================" "Package:    ${PACKAGE_ZIP}" "Repository: ${WORKDIR}" "Remote:     ${REMOTE}" ""
...
SOURCE_ROOT="$(find "$EXTRACT_DIR" -maxdepth 1 -type d -name 'sustainable-catalyst-platform-core-v2.21.0' | head -1)"
[ -n "$SOURCE_ROOT" ] && [ -f "$SOURCE_ROOT/backend/app/main.py" ] || { echo "ERROR: Expected v2.21.0 repository root was not found."; exit 1; }
if [ -d "$WORKDIR/.git" ]; then
 cd "$WORKDIR"; git remote set-url origin "$REMOTE"
 if ! git diff --quiet || ! git diff --cached --quiet || [ -n "$(git ls-files --others --exclude-standard)" ]; then git stash push -u -m "Automatic safety stash before Platform Core v2.21.0" >/dev/null || true; fi
...
cd "$WORKDIR"
"$PYTHON_BIN" scripts/validate_v2210_release.py
"$PYTHON_BIN" - <<'PY2'
from pathlib import Path
import hashlib,json
m=json.loads(Path('BUILD_MANIFEST.json').read_text()); assert m.get('release')=='2.21.0',m.get('release')
for i in m['files']:
...
bash -n PUSH_PLATFORM_CORE_V2210_FINAL.sh; bash -n deploy_and_validate_platform_core_v2_21_0_macos.sh
...
SMOKE_DB="$WORKDIR/backend/platform_core_v2210_push_smoke.db"; rm -f "$SMOKE_DB"
...
rm -f "$SMOKE_DB"
mkdir -p dist
(cd wordpress-plugin && zip -qr ../dist/sustainable-catalyst-platform-core-plugin-v2.21.0.zip sustainable-catalyst-platform-core)
cp backend/public_sdk/downloads/sc-platform-core-public-python-v2.21.0.zip dist/
cp backend/public_sdk/downloads/sc-platform-core-public-javascript-v2.21.0.zip dist/
git add -A
if git diff --cached --quiet; then echo "No repository changes need to be committed."; else git commit -m "Build Platform Core v2.21.0 multi-region resilience and failover coordination"; fi
git push --set-upstream origin "$BRANCH"
echo "PASS - Platform Core v2.21.0 pushed successfully"
if [ -n "${SC_CORE_PUBLIC_BASE_URL:-}" ]; then
 curl -fsS "${SC_CORE_PUBLIC_BASE_URL%/}/health" | "$PYTHON_BIN" -c 'import json,sys; d=json.load(sys.stdin); assert d.get("version")=="2.21.0",d; print("PASS - live /health")'
 curl -fsS "${SC_CORE_PUBLIC_BASE_URL%/}/v1/resilience/readiness" | "$PYTHON_BIN" -c 'import json,sys; d=json.load(sys.stdin); assert d.get("release")=="2.21.0",d; assert d.get("migration_0024_applied") is True,d; print("PASS - live resilience readiness")'

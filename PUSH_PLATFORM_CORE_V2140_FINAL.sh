...
PACKAGE_ZIP="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.14.0-repository.zip}"
WORKDIR="${2:-${HOME}/Downloads/sustainable-catalyst-platform-core-repo}"
EXTRACT_DIR="${TMPDIR:-/tmp}/sc-platform-core-v2140-extracted"
printf '%s\n' "============================================================" "PLATFORM CORE v2.14.0 — CROSS-PRODUCT EVIDENCE EXCHANGE" "============================================================" "Package:    ${PACKAGE_ZIP}" "Repository: ${WORKDIR}" "Remote:     ${REMOTE}" ""
...
SOURCE_ROOT="$(find "$EXTRACT_DIR" -maxdepth 1 -type d -name 'sustainable-catalyst-platform-core-v2.14.0' | head -1)"
[ -n "$SOURCE_ROOT" ] && [ -f "$SOURCE_ROOT/backend/app/main.py" ] || { echo "ERROR: Expected v2.14.0 repository root was not found."; exit 1; }
if [ -d "$WORKDIR/.git" ]; then
  cd "$WORKDIR"; git remote set-url origin "$REMOTE"
  if ! git diff --quiet || ! git diff --cached --quiet || [ -n "$(git ls-files --others --exclude-standard)" ]; then git stash push -u -m "Automatic safety stash before Platform Core v2.14.0" >/dev/null || true; fi
...
cd "$WORKDIR"
"$PYTHON_BIN" scripts/validate_v2140_release.py
"$PYTHON_BIN" scripts/validate_v2140_r1_promotion_repair.py
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
root=Path('.'); m=json.loads((root/'BUILD_MANIFEST.json').read_text()); assert m.get('release')=='2.14.0',m.get('release')
...
bash -n PUSH_PLATFORM_CORE_V2140_FINAL.sh; bash -n deploy_and_validate_platform_core_v2_14_0_macos.sh; bash -n repair_and_resume_platform_core_v2_14_0_r1_macos.sh
...
SMOKE_DB="$WORKDIR/backend/platform_core_v2140_push_smoke.db"; rm -f "$SMOKE_DB"
...
SC_CORE_DATABASE_URL="sqlite:///${SMOKE_DB}" SC_CORE_ENVIRONMENT=test backend/.venv/bin/python backend/scripts/validate_cross_product_exchange.py
...
rm -f "$SMOKE_DB"
mkdir -p dist
(cd wordpress-plugin && zip -qr ../dist/sustainable-catalyst-platform-core-plugin-v2.14.0.zip sustainable-catalyst-platform-core)
cp backend/public_sdk/downloads/sc-platform-core-public-python-v2.14.0.zip dist/
cp backend/public_sdk/downloads/sc-platform-core-public-javascript-v2.14.0.zip dist/
cp backend/public_sdk/postman/Sustainable_Catalyst_Public_API_v1.postman_collection.json dist/
git add -A
if git diff --cached --quiet; then echo "No repository changes need to be committed."; else git commit -m "Build Platform Core v2.14.0 cross-product evidence exchange"; fi
git push --set-upstream origin "$BRANCH"
echo "PASS - Platform Core v2.14.0 pushed successfully"
if [ -n "${SC_CORE_PUBLIC_BASE_URL:-}" ]; then
  curl -fsS "${SC_CORE_PUBLIC_BASE_URL%/}/health" | "$PYTHON_BIN" -c 'import json,sys; d=json.load(sys.stdin); assert d.get("version")=="2.14.0",d; print("PASS - live /health")'
  curl -fsS "${SC_CORE_PUBLIC_BASE_URL%/}/ready" | "$PYTHON_BIN" -c 'import json,sys; d=json.load(sys.stdin); assert d.get("ok") is True,d; assert d.get("earth_ocean_space_scientific_service_fabric")=="ready",d; assert d.get("cross_product_evidence_exchange")=="ready",d; print("PASS - live /ready")'
  curl -fsS -H "X-SC-API-Key: ${SC_CORE_WRITE_API_KEY:-}" "${SC_CORE_PUBLIC_BASE_URL%/}/v1/scientific-fabric/readiness" | "$PYTHON_BIN" -c 'import json,sys; d=json.load(sys.stdin); assert d.get("release")=="2.14.0",d; assert d.get("status")=="ready",d; assert d.get("truth_precedence")=="none",d; print("PASS - live scientific fabric readiness")'

  curl -fsS -H "X-SC-API-Key: ${SC_CORE_WRITE_API_KEY:-}" "${SC_CORE_PUBLIC_BASE_URL%/}/v1/exchange/readiness" | "$PYTHON_BIN" -c 'import json,sys; d=json.load(sys.stdin); assert d.get("release")=="2.14.0",d; assert d.get("status")=="ready",d; assert d.get("reference_first") is True,d; print("PASS - live exchange readiness")'
else

#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.26.0 validation stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR

BUNDLE="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.26.0-release-bundle.zip}"
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/sc-platform-core-v2260-bundle.XXXXXX")"; trap 'rm -rf "$WORK"' EXIT
unzip -q "$BUNDLE" -d "$WORK/bundle"; cd "$WORK/bundle"
[ -f SHA256SUMS.txt ] || { echo "ERROR: SHA256SUMS.txt missing"; exit 1; }
shasum -a 256 -c SHA256SUMS.txt
REPO_ZIP="sustainable-catalyst-platform-core-v2.26.0-repository.zip"; PLUGIN_ZIP="sustainable-catalyst-platform-core-v2.26.0-wordpress-plugin.zip"
unzip -t "$REPO_ZIP" >/dev/null; unzip -t "$PLUGIN_ZIP" >/dev/null
mkdir -p "$WORK/repo-extract"; unzip -q "$REPO_ZIP" -d "$WORK/repo-extract"
ROOT="$WORK/repo-extract/sustainable-catalyst-platform-core-v2.26.0"; [ -f "$ROOT/backend/app/main.py" ] || { echo "ERROR: repository root missing"; exit 1; }
cd "$ROOT"
"$PYTHON_BIN" scripts/validate_v2260_release.py
"$PYTHON_BIN" scripts/scan_push_safe_secrets.py .
"$PYTHON_BIN" - <<'PY'
import hashlib,json,pathlib
m=json.loads(pathlib.Path('BUILD_MANIFEST.json').read_text()); assert m['release']=='2.26.0'
for item in m['files']:
...
bash -n PUSH_PLATFORM_CORE_V2260_FINAL.sh; bash -n deploy_and_validate_platform_core_v2_26_0_macos.sh

if [ "${SC_CORE_BUNDLE_ONLY:-0}" = "1" ]; then echo "PASS - v2.26.0 bundle-only verification complete"; exit 0; fi
...
SMOKE_DB="$WORK/platform_core_v2260_smoke.db"; rm -f "$SMOKE_DB"; export SC_CORE_DATABASE_URL="sqlite:///${SMOKE_DB}" SC_CORE_ENVIRONMENT=test
...
rm -f "$SMOKE_DB"; echo "PASS - v2.26.0 deterministic validation complete"
if [ "${SC_CORE_VALIDATE_ONLY:-0}" = "1" ]; then exit 0; fi
cd "$WORK/bundle"; chmod +x PUSH_PLATFORM_CORE_V2260_FINAL.sh; ./PUSH_PLATFORM_CORE_V2260_FINAL.sh "$WORK/bundle/$REPO_ZIP"

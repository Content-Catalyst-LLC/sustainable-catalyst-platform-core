...
final class SC_Core_Investigation_Workspace_320 {
  ...
  public static function render($atts=[]){
    ...
    <section id="<?php echo esc_attr($id); ?>" class="sc-core-investigation-workspace" data-backend="<?php echo esc_attr(rtrim($base,'/')); ?>">
      <div class="sc-iw-head"><strong>Unified Investigation Workspace</strong><span>Platform Core v3.20.0.1</span></div>
    ...
    <script>(function(){const e=document.getElementById(<?php echo wp_json_encode($id); ?>);if(!e)return;const b=e.dataset.backend,s=e.querySelector('.sc-iw-status'),g=e.querySelector('.sc-iw-grid');if(!b){s.textContent='Backend URL is not configured.';return;}fetch(b+'/health',{credentials:'omit'}).then(r=>{if(!r.ok)throw new Error(r.status);return r.json()}).then(d=>{s.textContent='Unified Investigation Workspace · Core '+(d.version||'3.20.0.1')+' online';g.hidden=false}).catch(()=>{s.textContent='Investigation workspace backend unavailable.'})})();</script>

#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.37.0.1 validation stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR
BUNDLE="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.37.0.1-release-bundle.zip}"
select_python(){ local c; for c in python3.12 python3.11 python3.10 python3; do if command -v "$c" >/dev/null 2>&1 && "$c" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3,10) else 1)' >/dev/null 2>&1; then command -v "$c"; return 0; fi; done; echo "ERROR: Platform Core v2.37.0.1 requires Python 3.10+; Python 3.12 is preferred." >&2; return 1; }
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/sc-platform-core-v23701-bundle.XXXXXX")"; trap 'rm -rf "$WORK"' EXIT
unzip -q "$BUNDLE" -d "$WORK/bundle"; cd "$WORK/bundle"
[ -f SHA256SUMS.txt ] || { echo "ERROR: SHA256SUMS.txt missing"; exit 1; }
shasum -a 256 -c SHA256SUMS.txt
REPO_ZIP="sustainable-catalyst-platform-core-v2.37.0.1-repository.zip"; PLUGIN_ZIP="sustainable-catalyst-platform-core-v2.37.0.1-wordpress-plugin.zip"
unzip -t "$REPO_ZIP" >/dev/null; unzip -t "$PLUGIN_ZIP" >/dev/null
mkdir -p "$WORK/repo-extract"; unzip -q "$REPO_ZIP" -d "$WORK/repo-extract"
ROOT="$WORK/repo-extract/sustainable-catalyst-platform-core-v2.37.0.1"; [ -f "$ROOT/backend/app/main.py" ] || { echo "ERROR: repository root missing"; exit 1; }
cd "$ROOT"
"$PYTHON_BIN" scripts/validate_v2370_1_release.py
"$PYTHON_BIN" scripts/scan_push_safe_secrets.py .
"$PYTHON_BIN" - <<'PY2'
import hashlib,json,pathlib
m=json.loads(pathlib.Path('BUILD_MANIFEST.json').read_text()); assert m['release']=='2.37.0.1',m
for item in m['files']:
...
bash -n PUSH_PLATFORM_CORE_V2370_1_FINAL.sh; bash -n deploy_and_validate_platform_core_v2_37_0_1_macos.sh; bash -n DEPLOY_PLATFORM_CORE_V2370_1_CONTABO.sh
if [ "${SC_CORE_BUNDLE_ONLY:-0}" = "1" ]; then echo "PASS - v2.37.0.1 bundle-only verification complete"; exit 0; fi
...
SMOKE_DB="$WORK/platform_core_v23701_smoke.db"; rm -f "$SMOKE_DB"
...
backend/.venv/bin/python backend/scripts/migrate.py >/tmp/sc-core-v23701-migrate.json
PYTHONPATH=backend backend/.venv/bin/python backend/scripts/validate_causal_systems.py
backend/.venv/bin/python - <<'PY2'
import json
m=json.load(open('/tmp/sc-core-v23701-migrate.json')); assert '0041' in m['applied'] and m['pending']==[],m
...
echo "PASS - v2.37.0.1 release-critical validation complete"
if [ "${SC_CORE_VALIDATE_ONLY:-0}" = "1" ]; then exit 0; fi
cd "$WORK/bundle"; chmod +x PUSH_PLATFORM_CORE_V2370_1_FINAL.sh; PYTHON_BIN="$PYTHON_BIN" ./PUSH_PLATFORM_CORE_V2370_1_FINAL.sh "$WORK/bundle/$REPO_ZIP"

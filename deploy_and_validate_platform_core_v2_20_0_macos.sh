#!/usr/bin/env bash
set -euo pipefail
BUNDLE_INPUT="${1:-sustainable-catalyst-platform-core-v2.20.0-release-bundle.zip}"
BUNDLE="$(cd "$(dirname "$BUNDLE_INPUT")" && pwd)/$(basename "$BUNDLE_INPUT")"
WORK="${TMPDIR:-/tmp}/sc-platform-core-v2200-bundle"
...
REPO_ZIP="sustainable-catalyst-platform-core-v2.20.0-repository.zip"
PLUGIN_ZIP="sustainable-catalyst-platform-core-v2.20.0-wordpress-plugin.zip"
...
ROOT="repo-extract/sustainable-catalyst-platform-core-v2.20.0"
[ -f "$ROOT/backend/app/main.py" ] || { echo "ERROR: repository root missing"; exit 1; }
cd "$ROOT"
python3 scripts/validate_v2200_release.py
python3 - <<'PY2'
import hashlib,json,pathlib
m=json.loads(pathlib.Path('BUILD_MANIFEST.json').read_text()); assert m['release']=='2.20.0'
for i in m['files']:
...
bash -n PUSH_PLATFORM_CORE_V2200_FINAL.sh
bash -n deploy_and_validate_platform_core_v2_20_0_macos.sh
if [ "${SC_CORE_BUNDLE_ONLY:-0}" = "1" ]; then echo "PASS - v2.20.0 bundle-only verification complete"; exit 0; fi
...
SMOKE_DB="$WORK/platform_core_v2200_smoke.db"; rm -f "$SMOKE_DB"
...
rm -f "$SMOKE_DB"
echo "PASS - v2.20.0 deterministic validation complete"
if [ "${SC_CORE_VALIDATE_ONLY:-0}" = "1" ]; then exit 0; fi
cd "$WORK"; chmod +x PUSH_PLATFORM_CORE_V2200_FINAL.sh; ./PUSH_PLATFORM_CORE_V2200_FINAL.sh "$WORK/$REPO_ZIP"

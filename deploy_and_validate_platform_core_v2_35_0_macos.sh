#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.35.0 validation stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR

BUNDLE="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.35.0-release-bundle.zip}"
select_python() {
  local c
  ...
  echo "ERROR: Platform Core v2.35.0 requires Python 3.10+; Python 3.12 is preferred." >&2
  return 1
}
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/sc-platform-core-v2350-bundle.XXXXXX")"
...
bash -n PUSH_PLATFORM_CORE_V2350_FINAL.sh
bash -n deploy_and_validate_platform_core_v2_35_0_macos.sh
bash -n DEPLOY_PLATFORM_CORE_V2350_CONTABO.sh

if [ "${SC_CORE_BUNDLE_ONLY:-0}" = "1" ]; then
  echo "PASS - v2.35.0 bundle-only verification complete"
  exit 0
fi
...
# Release-critical validation. Full historical certification is opt-in.
...
SMOKE_DB="$WORK/platform_core_v2350_smoke.db"
rm -f "$SMOKE_DB"
export SC_CORE_DATABASE_URL="sqlite:///${SMOKE_DB}"
export SC_CORE_ENVIRONMENT=test
export SC_CORE_SCIENTIFIC_OBJECT_STORAGE_ROOT="$WORK/scientific-objects"
backend/.venv/bin/python backend/scripts/migrate.py >/tmp/sc-core-v2350-migrate.json
backend/.venv/bin/python backend/scripts/validate_scenario_compute_engine.py
backend/.venv/bin/python - <<'PY'
import json
m = json.load(open('/tmp/sc-core-v2350-migrate.json'))
assert '0038' in m['applied'] and m['pending'] == [], m
print('PASS - migration 0038 applied with pending=[]')
PY
...
echo "PASS - v2.35.0 release-critical validation complete"
if [ "${SC_CORE_VALIDATE_ONLY:-0}" = "1" ]; then
  exit 0
fi
cd "$WORK/bundle"
chmod +x PUSH_PLATFORM_CORE_V2350_FINAL.sh
./PUSH_PLATFORM_CORE_V2350_FINAL.sh "$WORK/bundle/$REPO_ZIP"

#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.24.0 validation stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR

BUNDLE="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.24.0-release-bundle.zip}"
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/sc-platform-core-v2240-bundle.XXXXXX")"
...
PLUGIN_ZIP="sustainable-catalyst-platform-core-v2.24.0-wordpress-plugin.zip"
unzip -t "$REPO_ZIP" >/dev/null
unzip -t "$PLUGIN_ZIP" >/dev/null

mkdir -p "$WORK/repo-extract"
unzip -q "$REPO_ZIP" -d "$WORK/repo-extract"
ROOT="$WORK/repo-extract/sustainable-catalyst-platform-core-v2.24.0"
...
if [ "${SC_CORE_BUNDLE_ONLY:-0}" = "1" ]; then
  echo "PASS - v2.24.0 bundle-only verification complete"
  exit 0
fi
...
SMOKE_DB="$WORK/platform_core_v2240_smoke.db"
rm -f "$SMOKE_DB"
...
backend/.venv/bin/python scripts/validate_v2240_r1_promotion_repair.py
...
rm -f "$SMOKE_DB"
echo "PASS - v2.24.0 deterministic validation complete"
...
cd "$WORK/bundle"
chmod +x PUSH_PLATFORM_CORE_V2240_FINAL.sh
./PUSH_PLATFORM_CORE_V2240_FINAL.sh "$WORK/bundle/$REPO_ZIP"

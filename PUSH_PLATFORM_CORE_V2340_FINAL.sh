#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.34.0 promotion stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR

PACKAGE_ZIP="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.34.0-repository.zip}"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-platform-core-v2340-push.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
unzip -q "$PACKAGE_ZIP" -d "$TMP"
SOURCE_ROOT="$TMP/sustainable-catalyst-platform-core-v2.34.0"
[ -f "$SOURCE_ROOT/backend/app/main.py" ] || { echo "ERROR: expected v2.34.0 source root missing"; exit 1; }
...
bash -n deploy_and_validate_platform_core_v2_34_0_macos.sh
bash -n DEPLOY_PLATFORM_CORE_V2340_CONTABO.sh

git add -A
if git diff --cached --quiet; then
  echo "No repository changes need to be committed."
else
  git commit -m "Build Platform Core v2.34.0 interactive model canvas"
fi
git push origin main

if git rev-parse 'v2.34.0' >/dev/null 2>&1; then
  echo "Tag v2.34.0 already exists locally."
else
  git tag -a v2.34.0 -m "Platform Core v2.34.0 — Interactive Model Canvas"
fi
git push origin v2.34.0

echo "PASS - Platform Core v2.34.0 pushed successfully"

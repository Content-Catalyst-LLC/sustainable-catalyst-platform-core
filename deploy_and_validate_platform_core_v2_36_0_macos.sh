...
select_python() {
  ...
  echo "ERROR: Platform Core v2.36.0 requires Python 3.10+; Python 3.12 is preferred." >&2
  return 1
}
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/sc-platform-core-v2360-bundle.XXXXXX")"
...
bash -n PUSH_PLATFORM_CORE_V2360_FINAL.sh
bash -n deploy_and_validate_platform_core_v2_36_0_macos.sh
bash -n DEPLOY_PLATFORM_CORE_V2360_CONTABO.sh

if [ "${SC_CORE_BUNDLE_ONLY:-0}" = "1" ]; then
  echo "PASS - v2.36.0 bundle-only verification complete"
  exit 0
fi
...
SMOKE_DB="$WORK/platform_core_v2360_smoke.db"
rm -f "$SMOKE_DB"
...
backend/.venv/bin/python backend/scripts/migrate.py >/tmp/sc-core-v2360-migrate.json
backend/.venv/bin/python backend/scripts/validate_uncertainty_reasoning.py
backend/.venv/bin/python - <<'PY'
import json
m = json.load(open('/tmp/sc-core-v2360-migrate.json'))
assert '0039' in m['applied'] and m['pending'] == [], m
print('PASS - migration 0039 applied with pending=[]')
PY
...
echo "PASS - v2.36.0 release-critical validation complete"
if [ "${SC_CORE_VALIDATE_ONLY:-0}" = "1" ]; then
  exit 0
fi
cd "$WORK/bundle"
chmod +x PUSH_PLATFORM_CORE_V2360_FINAL.sh
./PUSH_PLATFORM_CORE_V2360_FINAL.sh "$WORK/bundle/$REPO_ZIP"

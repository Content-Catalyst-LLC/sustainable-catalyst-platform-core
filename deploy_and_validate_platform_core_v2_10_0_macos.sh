#!/usr/bin/env bash
set -euo pipefail
BUNDLE="${1:-sustainable-catalyst-platform-core-v2.10.0-release-bundle.zip}"
WORK="${TMPDIR:-/tmp}/sc-platform-core-v2100-bundle"
...
REPO_ZIP="sustainable-catalyst-platform-core-v2.10.0-repository.zip"; PLUGIN_ZIP="sustainable-catalyst-platform-core-v2.10.0-wordpress-plugin.zip"
...
ROOT="repo-extract/sustainable-catalyst-platform-core-v2.10.0"; [ -f "$ROOT/backend/app/main.py" ] || { echo "ERROR: repository root missing"; exit 1; }
cd "$ROOT"; python3 scripts/validate_v2100_release.py
python3 -c 'import hashlib,json,pathlib; m=json.loads(pathlib.Path("BUILD_MANIFEST.json").read_text()); assert m["release"]=="2.10.0"; [(lambda p,i: (_ for _ in ()).throw(AssertionError(i["path"])) if (not p.is_file() or hashlib.sha256(p.read_bytes()).hexdigest()!=i["sha256"]) else None)(pathlib.Path(i["path"]),i) for i in m["files"]]; print("PASS - clean extraction manifest verified:",m["file_count"],"files")'
...
if [ "${SC_CORE_BUNDLE_ONLY:-0}" = "1" ]; then echo "PASS - v2.10.0 bundle-only verification complete"; exit 0; fi
...
SMOKE_DB="$WORK/platform_core_v2100_smoke.db"; rm -f "$SMOKE_DB"
...
rm -f "$SMOKE_DB"; echo "PASS - v2.10.0 deterministic validation complete"
if [ "${SC_CORE_VALIDATE_ONLY:-0}" = "1" ]; then exit 0; fi
cd "$WORK"; chmod +x PUSH_PLATFORM_CORE_V2100_FINAL.sh; ./PUSH_PLATFORM_CORE_V2100_FINAL.sh "$WORK/$REPO_ZIP"

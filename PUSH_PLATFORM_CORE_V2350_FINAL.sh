#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.35.0 promotion stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR

PACKAGE_ZIP="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.35.0-repository.zip}"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-platform-core-v2350-push.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$PACKAGE_ZIP" -d "$TMP"
SOURCE_ROOT="$TMP/sustainable-catalyst-platform-core-v2.35.0"
[ -f "$SOURCE_ROOT/backend/app/main.py" ] || { echo "ERROR: expected v2.35.0 source root missing"; exit 1; }
...
if git diff --cached --quiet; then
  ...
  git commit -m "Build Platform Core v2.35.0 scenario compute engine"
fi
git push origin main

if git rev-parse 'v2.35.0' >/dev/null 2>&1; then
  echo "Tag v2.35.0 already exists locally."
else
  git tag -a v2.35.0 -m "Platform Core v2.35.0 — Scenario Compute Engine"
fi
git push origin v2.35.0

echo "PASS - Platform Core v2.35.0 pushed successfully"

#!/usr/bin/env bash
set -euo pipefail
BUNDLE="${1:-sustainable-catalyst-platform-core-v2.18.0-release-bundle.zip}"
WORK="${TMPDIR:-/tmp}/sc-platform-core-v2180-bundle"
...
REPO_ZIP="sustainable-catalyst-platform-core-v2.18.0-repository.zip"; PLUGIN_ZIP="sustainable-catalyst-platform-core-v2.18.0-wordpress-plugin.zip"
...
ROOT="repo-extract/sustainable-catalyst-platform-core-v2.18.0"; [ -f "$ROOT/backend/app/main.py" ] || { echo "ERROR: repository root missing"; exit 1; }
cd "$ROOT"; python3 scripts/validate_v2180_release.py
python3 scripts/validate_v2180_r1_promotion_repair.py
python3 - <<'PY2'
import hashlib,json,pathlib
m=json.loads(pathlib.Path('BUILD_MANIFEST.json').read_text()); assert m['release']=='2.18.0'
for i in m['files']:
...
if [ "${SC_CORE_BUNDLE_ONLY:-0}" = "1" ]; then echo "PASS - v2.18.0 bundle-only verification complete"; exit 0; fi
...
SMOKE_DB="$WORK/platform_core_v2180_smoke.db"; rm -f "$SMOKE_DB"; export SC_CORE_DATABASE_URL="sqlite:///${SMOKE_DB}" SC_CORE_ENVIRONMENT=test
...
rm -f "$SMOKE_DB"; echo "PASS - v2.18.0 deterministic validation complete"
if [ "${SC_CORE_VALIDATE_ONLY:-0}" = "1" ]; then exit 0; fi
cd "$WORK"; chmod +x PUSH_PLATFORM_CORE_V2180_FINAL.sh; ./PUSH_PLATFORM_CORE_V2180_FINAL.sh "$WORK/$REPO_ZIP"

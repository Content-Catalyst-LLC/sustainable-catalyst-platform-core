#!/usr/bin/env bash
set -euo pipefail
trap 'rc=$?; echo "ERROR: v2.24.0 promotion stopped at line $LINENO (exit $rc): $BASH_COMMAND" >&2; exit $rc' ERR

ORG="Content-Catalyst-LLC"
REPO="sustainable-catalyst-platform-core"
BRANCH="main"
REMOTE="git@github.com:${ORG}/${REPO}.git"
PACKAGE_ZIP="${1:-${HOME}/Downloads/sustainable-catalyst-platform-core-v2.24.0-repository.zip}"
WORKDIR="${2:-${HOME}/Downloads/sustainable-catalyst-platform-core-repo}"
EXTRACT_DIR="${TMPDIR:-/tmp}/sc-platform-core-v2240-extracted"

printf '%s\n' \
  "============================================================" \
  "PLATFORM CORE v2.24.0 — CAPACITY FORECASTING / RESOURCE GOVERNANCE" \
  "============================================================" \
  "Package:    ${PACKAGE_ZIP}" \
  "Repository: ${WORKDIR}" \
  "Remote:     ${REMOTE}" ""
...
SOURCE_ROOT="$(find "$EXTRACT_DIR" -maxdepth 1 -type d -name 'sustainable-catalyst-platform-core-v2.24.0' | head -1)"
[ -n "$SOURCE_ROOT" ] && [ -f "$SOURCE_ROOT/backend/app/main.py" ] || { echo "ERROR: Expected v2.24.0 repository root was not found."; exit 1; }

if [ -d "$WORKDIR/.git" ]; then
  cd "$WORKDIR"
  git remote set-url origin "$REMOTE"
  if ! git diff --quiet || ! git diff --cached --quiet || [ -n "$(git ls-files --others --exclude-standard)" ]; then
    git stash push -u -m "Automatic safety stash before Platform Core v2.24.0" >/dev/null || true
  fi
...
"$PYTHON_BIN" scripts/validate_v2240_release.py
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib, json
m = json.loads(Path('BUILD_MANIFEST.json').read_text())
assert m.get('release') == '2.24.0', m.get('release')
for item in m['files']:
    p = Path(item['path'])
...
"$PYTHON_BIN" scripts/validate_v2240_r1_promotion_repair.py
...
mkdir -p dist
(cd wordpress-plugin && zip -qr ../dist/sustainable-catalyst-platform-core-plugin-v2.24.0.zip sustainable-catalyst-platform-core)
cp backend/public_sdk/downloads/sc-platform-core-public-python-v2.24.0.zip dist/
cp backend/public_sdk/downloads/sc-platform-core-public-javascript-v2.24.0.zip dist/

git add -A
if git diff --cached --quiet; then
  echo "No repository changes need to be committed."
else
  git commit -m "Repair Platform Core v2.24.0 R1 secret-scan promotion lineage"
fi
git push --set-upstream origin "$BRANCH"
echo "PASS - Platform Core v2.24.0 pushed successfully"

if [ -n "${SC_CORE_PUBLIC_BASE_URL:-}" ]; then
  curl -fsS "${SC_CORE_PUBLIC_BASE_URL%/}/health" | "$PYTHON_BIN" -c 'import json,sys; d=json.load(sys.stdin); assert d.get("version")=="2.24.0",d; print("PASS - live /health")'
  curl -fsS "${SC_CORE_PUBLIC_BASE_URL%/}/v1/federation/readiness" | "$PYTHON_BIN" -c 'import json,sys; d=json.load(sys.stdin); assert d.get("release")=="2.24.0",d; assert d.get("migration_0026_applied") is True,d; print("PASS - live federation readiness")'
  curl -fsS "${SC_CORE_PUBLIC_BASE_URL%/}/v1/capacity/readiness" | "$PYTHON_BIN" -c 'import json,sys; d=json.load(sys.stdin); assert d.get("release")=="2.24.0",d; assert d.get("migration_0027_applied") is True,d; assert d.get("automatic_scaling") is False,d; print("PASS - live capacity readiness")'

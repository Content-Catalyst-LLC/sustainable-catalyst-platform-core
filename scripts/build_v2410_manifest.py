...
EXCLUDE_PREFIXES={'.git','backend/.venv'}
...
out={'release':'2.41.0','file_count':len(files),'files':files}; (ROOT/'BUILD_MANIFEST.json').write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print(f"PASS - v2.41.0 manifest written: {len(files)} files")

...
for p in sorted(R.rglob('*')):
 ...
 if p.name in {'BUILD_MANIFEST.json'} or p.suffix in {'.pyc','.db'}: continue
 rel=p.relative_to(R).as_posix(); files.append({'path':rel,'sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'bytes':p.stat().st_size})
payload={'release':'2.17.0','file_count':len(files),'files':files}; (R/'BUILD_MANIFEST.json').write_text(json.dumps(payload,indent=2)+'\n')
print(f"PASS - v2.17.0 manifest built: {len(files)} files")
